...
Query `Engine::MaxDraftTokensPerStep` (`OgaEngineMaxDraftTokensPerStep` or
`Engine.max_draft_tokens_per_step`) before proposing work. Zero means that the decoder does not
...
The request must already belong to the Engine. Verification currently supports only requests whose
resolved search mode is greedy (`do_sample` is false, `top_k` is 1, or `temperature` is zero), with
no guidance, repetition penalty, no-repeat-ngram processing, or active minimum-length processing.
Passing an empty sequence clears a pending proposal.
...
add several entries to the request's unseen-output stream even though `Engine::Step()` still returns
one ready request handle at a time.
...
A committed step consumes the complete proposal, including drafts omitted by budgeting or a
prefill step. A retryable rollback restores the request and leaves the proposal pending for retry.
During commit, the cache reservation is narrowed from the scheduled K+1 slots to the accepted
prefix plus the request's mandatory token; paged KV and fixed recurrent state publish at that same
boundary.
