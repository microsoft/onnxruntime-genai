...
void StreamingTranscribe(const std::string& model_path, const std::string& audio_path, bool disable_vad = false) {
  ...
  if (disable_vad) {
    ...
    std::cout << "  VAD: disabled (--disable_vad)" << std::endl;
  } else if (processor->IsVadEnabled()) {
    std::cout << "  VAD: enabled" << std::endl;
  } else {
    std::cout << "  VAD: disabled (silero_vad.onnx not found in model dir)" << std::endl;
  }
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (std::string(argv[i]) == "--model_path" && i + 1 < argc) {
      ...
      audio_file = argv[++i];
    } else if (std::string(argv[i]) == "--disable_vad") {
      disable_vad = true;
    }
  }
  ...
  try {
    StreamingTranscribe(model_path, audio_file, disable_vad);
