...
def next_user_fragment(content):
    return f"<|im_end|>\n<|im_start|>user\n{content}<|im_end|>\n<|im_start|>assistant\n"


def tool_result_fragment(tool_result):
    return next_user_fragment(f"<tool_response>\n{json.dumps(tool_result)}\n</tool_response>")
...
def run_agent(args, workspace):
    ...
    for step in range(1, args.max_tool_rounds * 2 + 2):
        output, _ = generate(engine, request, tokenizer)
        ...
        if tool_call is None:
            if tests_passed:
                ...
                return
            checkpoint_preview = output[:500]
            if len(output) > len(checkpoint_preview):
                checkpoint_preview += " [truncated]"
            print(f"Agent output {step}: {checkpoint_preview!r}")
            recovery_prompt = (
            ...
            continuation_fragment = next_user_fragment(recovery_prompt)
            ...
            continuation_fragment = tool_result_fragment(tool_result)
        ...
        continuation_tokens = tokenizer.encode(continuation_fragment)
