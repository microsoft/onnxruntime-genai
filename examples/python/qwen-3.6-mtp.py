...
"""Qwen3.x MTP (multi-token prediction) self-speculative decoding example.
...
The main decoder and MTP head may be supplied as separate model directories with ``-m`` and
``-d``, or as one packaged directory with ``--model_path``. The one-directory layout derives
the MTP session configuration from the ``model.mtp`` section of ``genai_config.json``.
