...
def _get_model_session_options_overlay(path: str, ep: str, ep_options: dict[str, str]) -> dict[str, Any]:
    ...
    model_overlay = {}
    for component in ["decoder", "draft", "encoder", "vision", "speech", "embedding", "joiner", "vad", "mtp"]:
...
def _get_effective_ep_options(ep_options: dict[str, str], search_options: dict[str, int]) -> dict[str, str]:
...
def get_config(
    ...
    if not ep_path and ep != "follow_config":
        ...
        model_provider_overlay = _get_model_session_options_overlay(path, ep, effective_ep_options)
        if ep != "cpu":
