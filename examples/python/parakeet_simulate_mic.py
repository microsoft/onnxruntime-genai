...
SAMPLE_RATE = 16000
CHUNK_SAMPLES = 12800  # 0.8 seconds at 16kHz (NeMo recommended default)
...
def simulate_microphone(model_path, audio_path, realtime=True):
    ...
    for i in range(0, len(audio), CHUNK_SAMPLES):
        # Wait for real-time moment (simulate microphone latency)
        if realtime:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-realtime", action="store_true",
                        help="Skip wall-clock delays (process as fast as possible)")
    args = parser.parse_args()

    simulate_microphone(args.model_path, args.audio_file, realtime=not args.no_realtime)
