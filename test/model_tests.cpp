...
std::vector<float> RunFirstDecodeWithCudaGraph(bool enable_cuda_graph) {
  auto config = OgaConfig::Create(c_cuda_graph_model_path);
...
TEST(ModelTests, RecurrentStateCudaGraphCaptureMatchesEager) {
  ...
  const auto eager_logits = RunFirstDecodeWithCudaGraph(false);
  const auto graph_logits = RunFirstDecodeWithCudaGraph(true);
