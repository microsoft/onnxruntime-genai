...
TEST_F(EngineStepTest, CompositeChunkFailureRetriesAndContinuesResidentState) {
  model_ = LoadSyntheticCompositeModel();
  auto engine = MakeCompositeDoublesEngine(model_, EosToken(*model_));
  ...
  try {
    static_cast<void>(engine.engine->Step());
    FAIL() << "Expected the second prefill chunk to fail.";
  } catch (const EngineStepError& error) {
    EXPECT_EQ(error.Outcome().kind, StepOutcomeKind::RetryableBatchAbort);
  }
  ...
  // Retry starts from the first committed chunk, completes the prompt, and samples EOS.
  ASSERT_EQ(engine.engine->Step(), request);
  EXPECT_EQ(request->status_, RequestStatus::TurnComplete);
  ...
  // Continue reuses the same resident fixed slot and chunks the appended turn.
...
TEST_F(EngineStepTest, PlanningAllocationFailureDoesNotPoisonEngine) {
  auto engine = MakeDoublesEngine(model_, /*capacity=*/4, EosToken(*model_));
  auto request = MintRequest(*model_, Prompt(10));
  engine.engine->AddRequest(request);
  engine.cache->ThrowPlanningBadAllocOnce();

  EXPECT_THROW(static_cast<void>(engine.engine->Step()), std::bad_alloc);
  EXPECT_NO_THROW(static_cast<void>(engine.engine->Step()));
}

TEST_F(EngineStepTest, PlanningConsistencyFailurePoisonsEngine) {
  auto engine = MakeDoublesEngine(model_, /*capacity=*/4, EosToken(*model_));
  auto request = MintRequest(*model_, Prompt(10));
  engine.engine->AddRequest(request);
  ...
  try {
    static_cast<void>(engine.engine->Step());
    FAIL() << "Expected a fatal planning consistency failure.";
  } catch (const EngineStepError& error) {
    EXPECT_EQ(error.Outcome().kind,
...
TEST_F(EngineStepTest, OverselectedTokenBudgetPoisonsEngine) {
  model_->config_->engine.dynamic_batching->max_scheduled_tokens = 1;
  auto engine = MakeDoublesEngine(model_, /*capacity=*/4, EosToken(*model_));
  auto first = MintRequest(*model_, Prompt(10));
  auto second = MintRequest(*model_, Prompt(11));
  engine.engine->AddRequest(first);
  engine.engine->AddRequest(second);
  ...
  try {
    static_cast<void>(engine.engine->Step());
    FAIL() << "Expected a fatal token-budget planning failure.";
  } catch (const EngineStepError& error) {
    EXPECT_EQ(error.Outcome().kind,
              StepOutcomeKind::ExecutionContractFailure);
  }
...
TEST_F(EngineStepTest, CommitPreparationFailureRestoresRequestStateBeforeFatal) {
  ...
  try {
    static_cast<void>(engine.engine->Step());
    FAIL() << "Expected a fatal transaction preparation failure.";
  } catch (const EngineStepError& error) {
    EXPECT_EQ(error.Outcome().kind,
              StepOutcomeKind::ExecutionContractFailure);
  }
...
TEST_F(EngineStepTest, BudgetedOutDraftMatchesCompositeFixedStateReservation) {
  model_ = LoadSyntheticCompositeModel();
  model_->config_->engine.dynamic_batching->num_blocks = 1;
  model_->config_->engine.dynamic_batching->max_batch_size = 1;
  model_->config_->engine.dynamic_batching->max_scheduled_tokens = 1;
  const int32_t eos = EosToken(*model_);
  const int32_t filler = eos == 5 ? 6 : 5;
  auto engine = MakeCompositeDoublesEngine(model_, filler);
  ...
  ASSERT_EQ(engine.engine->Step(), request);
  while (request->HasUnseenTokens()) request->UnseenToken();
  ASSERT_EQ(request->CurrentSequenceLength(), 4);
  ASSERT_EQ(request->ProcessedSequenceLength(), 3);
  ...
  engine.executor->SetExecutionCallback([&](ExecutionContext& context) {
    ...
    EXPECT_EQ(context.plan->fixed_state.staging_bytes,
              context.fixed_state_staging_bytes);
  });
  EXPECT_NO_THROW(static_cast<void>(engine.engine->Step()));
  EXPECT_EQ(request->PendingDraftTokenCount(), 0u);
}

TEST_F(EngineStepTest, DraftBeyondPagedCapacityFallsBackToBaseDecode) {
  model_ = LoadSyntheticCompositeModel();
  model_->config_->engine.dynamic_batching->num_blocks = 1;
  model_->config_->engine.dynamic_batching->max_batch_size = 1;
  model_->config_->engine.dynamic_batching->max_scheduled_tokens = 2;
