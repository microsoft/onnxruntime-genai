...
// Tiny GQA model for CUDA graph capture tests (GQA + shared recurrent state).
// Generated by test/python/create_gqa_model.py
static const char* c_cuda_graph_model_path = MODEL_PATH "hf-internal-testing/tiny-qwen35-cuda";
static const char* c_legacy_recurrent_model_path = MODEL_PATH "hf-internal-testing/tiny-qwen35-legacy-cuda";

std::vector<float> RunFirstDecodeWithCudaGraph(const char* model_path, bool enable_cuda_graph) {
  auto config = OgaConfig::Create(model_path);
  config->ClearProviders();
  config->AppendProvider("cuda");
  config->SetProviderOption("cuda", "enable_cuda_graph", enable_cuda_graph ? "1" : "0");
  ...
  auto generator = OgaGenerator::Create(*model, *params);
  generator->AppendTokens(std::vector<int32_t>{10, 20, 30, 40});
  generator->GenerateNextToken();

  auto logits_tensor = generator->GetLogits();
  auto* logits = reinterpret_cast<float*>(logits_tensor->Data());
  return {logits, logits + 1000};
}

TEST(ModelTests, LegacyRecurrentStateBindingDefaultsRunFirstDecode) {
  if (!std::filesystem::exists(c_legacy_recurrent_model_path)) {
    GTEST_SKIP() << "Legacy recurrent-state model not available at: " << c_legacy_recurrent_model_path;
  }

  const auto logits = RunFirstDecodeWithCudaGraph(c_legacy_recurrent_model_path, false);
  EXPECT_EQ(logits.size(), 1000u);
}

TEST(ModelTests, RecurrentStateCudaGraphCaptureMatchesEager) {
  ...
  const auto eager_logits = RunFirstDecodeWithCudaGraph(c_cuda_graph_model_path, false);
  const auto graph_logits = RunFirstDecodeWithCudaGraph(c_cuda_graph_model_path, true);
  ASSERT_EQ(eager_logits.size(), graph_logits.size());
  for (size_t i = 0; i < eager_logits.size(); ++i) {
    EXPECT_NEAR(eager_logits[i], graph_logits[i], 1e-3f) << "logit index " << i;
  }
}
