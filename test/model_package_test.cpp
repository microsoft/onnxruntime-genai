...
// --- Drift-detection tests for variant.json session_options field coverage ---
//
// These tests exist to catch the case where someone adds a new typed field to
// Config::SessionOptions and updates the genai_config.json JSON parser but forgets to
// update ApplyVariantFileOptions in model_package.cpp (or vice versa). They load a
// "kitchen-sink" package whose variant.json declares a value for every typed SO field,
// then assert the merged Config carries those values where expected. Pairs with the
// follow-up X-macro design described in p/drift-prevention-followup.md.
//
// The fixture's variant.json sets every typed SO field to a 'variant-*' value, and its
// consumer_metadata.genai_config_overlay re-sets a subset of those fields to 'genai-*'
// values. So one load exercises both halves of the merge contract:
//   * overlay-set fields must carry the genai-* value (genai_config wins on conflict)
//   * variant-only fields must carry the variant-* value (variant fills gaps)

static const char* kKitchenSinkPkgPath = MODEL_PATH "kitchen-sink-cpu.ortpackage";

TEST(ModelPackageSessionOptionsDrift, MergesAllTypedSoFields) {
  ...
  EXPECT_EQ(*so.log_id, "genai-log-id");
  ...
  ASSERT_TRUE(so.log_verbosity_level.has_value());
  ...
  EXPECT_EQ(*so.enable_profiling, "/tmp/variant-profile");

  ASSERT_TRUE(so.custom_ops_library.has_value());
  // custom_ops_library is conditionally rewritten to absolute when the file exists in the
  // variant dir. The fixture doesn't ship the .so, so it stays as the literal variant value.
  EXPECT_EQ(*so.custom_ops_library, "libcustomops.so");

  // Unknown-key fallback: arbitrary keys land in config_entries (variant fills since the
  // overlay doesn't override).
