...
class TestSpeculativeGeneration:
    ...
    @pytest.mark.parametrize(("model_type", "threshold"), [("phi3", 4097), ("phimoe", 4097), ("phi3small", 8193)])
    def test_phi3_with_draft_reprefills_at_rope_threshold(self, tmp_path, model_type, threshold):
        path = _make_tiny_draft_spec_model(tmp_path / model_type, model_type, threshold + 8)
        model = og.Model(path)
        params = og.GeneratorParams(model)
...
def _guided_tokens(
    model_path, gtype, gdata, max_length, k=None, want_stats=False, repetition_penalty=None, min_length=None
):
...
def _sampled_guided_tokens(model_path, gtype, gdata, max_length, seed, k=None, temperature=1.0, top_p=1.0, top_k=0):
