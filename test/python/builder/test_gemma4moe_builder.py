...
class TestGemma4MoEQuarkPath:
    """Pre-quantized Quark uint2 (2-bit) expert path: make_moe_quark_preprocessing + subgraph."""

    def _make_quark_model(self, ep="cpu", group_size=16):
        m = _make_model(onnx_dtype=ir.DataType.FLOAT)
        # The QuarkModel loader sets these; seed them so the quark path can run standalone.
        m.quant_type = "quark"
        m.ep = ep
    ...
    def _build_quark_layer(self, m, group_size=16):
        ...
        # Drive the MoE-branch sequence that make_moe runs for the quark path.
        m.make_moe_preprocessing(0, layer)
        router_probs = m.make_moe_router(0, layer, "resid")
        m.make_moe_subgraph(0, layer, "resid", router_probs)
        return layer
...
class TestGemma4MoEQuarkInt4Path:
    """Plain Quark/AWQ int4 expert path: the shape-guarded generalization of the quark MoE path
...
class TestGemma4MoEModel:
    ...
    def test_qmoe_under_int4(self):
        ...
        assert _make_model(onnx_dtype=ir.DataType.INT4).moe_attrs["op_type"] == "QMoE"
        assert _make_model(onnx_dtype=ir.DataType.FLOAT).moe_attrs["op_type"] == "MoE"
