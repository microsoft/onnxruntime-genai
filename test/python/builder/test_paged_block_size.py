...
tile makes ORT fall back to another backend rather than reject the model. Which tile
applies depends on the head size of each model in the export, so the builder leaves the
choice to the caller and enforces only the op-level rule.
"""
