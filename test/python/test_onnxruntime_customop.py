...
# CUSTOM_OP_PATH = '/home/leca/code/onnxruntime-genai/test/custom_ops/build/libgenai_custom_ops_test.so'
CUSTOM_OP_PATH = "/home/yingxiong/projects/onnxruntime-genai/test/custom_ops/build/libgenai_custom_ops_test.so"
...
def create_pagedattention_test_model(
    batch_size,
    slot_cnt_per_block,
    block_cnt_per_layer,
    block_cnt_needed_by_longest_seq,
    num_heads=32,
    num_kv_heads=32,
    head_size=16,
    scale=0.0,
    ...
    key_cache = helper.make_tensor_value_info(
        ...
        [block_cnt_per_layer, num_kv_heads * head_size * slot_cnt_per_block],
    )
    ...
    block_tables = helper.make_tensor_value_info(
        ...
        [batch_size, block_cnt_needed_by_longest_seq],
    )
    ...
    graph = helper.make_graph(nodes, "test_paged_attention", graph_input, [attn_out])
    model = make_onnx_model(graph)
    return model


def create_ort_session(
    custom_op_shared_lib_path,
    batch_size,
    slot_cnt_per_block,
    block_cnt_per_layer,
    block_cnt_needed_by_longest_seq,
    ...
    onnx_model = create_pagedattention_test_model(
        batch_size,
        slot_cnt_per_block,
        block_cnt_per_layer,
        block_cnt_needed_by_longest_seq,
...
class TestPagedAttentionOp(unittest.TestCase):
    def kv_cache_populated_correctly(
        self,
        key,
        value,
        key_cache,
        ...
        for token_idx in range(key.shape[0]):
            ...
            for embed_idx in range(key.shape[1]):
                self.assertTrue(
                    np.abs(
                        key[token_idx][embed_idx]
                        - key_cache[block_idx][
                            block_offset * num_kv_heads * head_size + embed_idx
                        ]
                    )
                    <= 0.001,
    ...
    def test_cuda_paged_attention_prompt_only(self):
        sess = create_ort_session(
            CUSTOM_OP_PATH,
            batch_size=3,
            slot_cnt_per_block=16,
            block_cnt_per_layer=32,
            block_cnt_needed_by_longest_seq=8,
        )

        query = np.random.randn(381, 512).astype(
            np.float16
        )  # 381 is the token num of all the sequences (127, 127, 127)
        key = np.random.randn(381, 512).astype(np.float16)
        value = np.random.randn(381, 512).astype(np.float16)
        key_cache = np.zeros([32, 8192]).astype(np.float16)
        value_cache = np.zeros([32, 8192]).astype(np.float16)
        ...
        ).astype(np.int32)  # each sequence occupies 8 blocks (127/16)
        slot1 = np.arange(0, 127, dtype=np.int32)
        slot2 = np.arange(128, 255, dtype=np.int32)
        slot3 = np.arange(256, 383, dtype=np.int32)
        slot_mappings = np.concatenate((slot1, slot2, slot3))
    ...
    def test_cuda_paged_attention_decoding_only(self):
        ...
        sess = create_ort_session(
            ...
            slot_cnt_per_block=paged_kv_block_size,
            block_cnt_per_layer=6,
            block_cnt_needed_by_longest_seq=3,
            num_heads=nheads,
            num_kv_heads=nheads,
            head_size=d,
        )
        ...
        key_cache_6x256x6x16 = np.random.randn(
            6, paged_kv_block_size, nheads, d
        ).astype(np.float16)
        value_cache_6x256x6x16 = np.random.randn(
            6, paged_kv_block_size, nheads, d
        ).astype(np.float16)
        key_cache = key_cache_6x256x6x16.reshape(6, paged_kv_block_size * nheads * d)
        value_cache = value_cache_6x256x6x16.reshape(
            6, paged_kv_block_size * nheads * d
        )
        ...
        k_cache_cpu = rearrange(
            key_cache_pt[block_tables_pt.flatten()],
            "(b nblocks) block_size ... -> b (nblocks block_size) ...",
            b=batch_size,
        )[:, :seqlen_k]
        v_cache_cpu = rearrange(
            value_cache_pt[block_tables_pt.flatten()],
            "(b nblocks) block_size ... -> b (nblocks block_size) ...",
            b=batch_size,
        )[:, :seqlen_k]
        ...
        update_mask = torch.logical_and(
            cache_seqlens_expand <= arange, arange < cache_seqlens_expand + 1
        )
        k_cache_cpu[update_mask] = rearrange(k, "b s ... -> (b s) ...")
        v_cache_cpu[update_mask] = rearrange(v, "b s ... -> (b s) ...")
        out_ref, _ = attention_ref(
            q, k_cache_cpu, v_cache_cpu, None, key_padding_mask, 0.0, None, causal=True
        )
    ...
    def test_cuda_paged_attention_prompt_rotary_embedding(self):
        ...
        slot_mappings = np.concatenate((slot1, slot2, slot3))
        context_lens = np.array([127, 127, 127]).astype(np.int32)
        cos_sin_cache = np.random.uniform(
            -1, 1, size=[np.max(context_lens), 16]
        ).astype(
            np.float16
        )  # uniform distribution in the range [-1, 1) for cos/sin value in the size of [max_seq_len, head_size]
        is_prompt = np.array([1]).astype(np.int32)
        y = sess.run(
            None,
            {
    ...
    def test_cuda_paged_attention_prompt_check_kvcache(self):
        ...
        io_binding.bind_output("attn_out")
        sess.run_with_iobinding(io_binding)
        self.kv_cache_populated_correctly(
            key,
            value,
            key_cache_ort.numpy(),
            value_cache_ort.numpy(),
            slot_mappings,
