...
if(ORT_HOME)
  ...
  elseif (CMAKE_SYSTEM_NAME MATCHES "AIX")
    ...
    find_package(onnxruntime QUIET PATHS ${ORT_HOME}/${CMAKE_BUILD_TYPE} ${ORT_HOME} NO_DEFAULT_PATH)
    if(onnxruntime_FOUND)
      get_target_property(ORT_HEADER_DIR onnxruntime::onnxruntime INTERFACE_INCLUDE_DIRECTORIES)
      _get_target_location_property(_onnxruntime_location onnxruntime::onnxruntime IMPORTED_LOCATION)
      if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
        _get_target_location_property(_onnxruntime_lib onnxruntime::onnxruntime IMPORTED_IMPLIB)
      else()
        set(_onnxruntime_lib "${_onnxruntime_location}")
      endif()
    else()
      find_file(_onnxruntime_c_api_h onnxruntime_c_api.h REQUIRED
              HINTS "${ORT_HOME}" PATH_SUFFIXES include include/onnxruntime)
      if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
        find_file(_onnxruntime_location onnxruntime.dll REQUIRED
                HINTS "${ORT_HOME}" PATH_SUFFIXES bin lib)
        find_file(_onnxruntime_lib onnxruntime.lib REQUIRED
                HINTS "${ORT_HOME}" PATH_SUFFIXES lib)
      else()
        find_file(_onnxruntime_location ${CMAKE_SHARED_LIBRARY_PREFIX}onnxruntime${CMAKE_SHARED_LIBRARY_SUFFIX} REQUIRED
                HINTS "${ORT_HOME}" PATH_SUFFIXES bin lib)
        set(_onnxruntime_lib "${_onnxruntime_location}")
      endif()
      cmake_path(GET _onnxruntime_c_api_h PARENT_PATH ORT_HEADER_DIR)
    endif()
    if(NOT _onnxruntime_location)
      message(FATAL_ERROR "Unable to locate the ONNXRuntime installation or build.")
    endif()
    cmake_path(GET _onnxruntime_location PARENT_PATH ORT_BIN_DIR)
