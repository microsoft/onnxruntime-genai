...
void GptSubgraph::CreateInitialFeeds(
  ...
  assert(session_state_ != nullptr); // "Setup must be called before CreateInitialFeeds");

//  const IExecutionProvider* provider = GetProvider();

  TensorShape input_ids_shape = input_ids.GetTensorTypeAndShapeInfo()->GetShape();
  assert(input_ids_shape.NumDimensions() == 2);
  ...
  OrtAllocator* cpu_allocator = allocator_; // session_state_->GetAllocator(input_ids.Location());

  // Store allocator, which will be used in remaining feeds
  auto &default_allocator = Ort::Allocator::GetWithDefaultOptions(); //session_state_->GetAllocator(provider->GetOrtDeviceByMemType(OrtMemTypeDefault));
  ...
  create_gpt_inputs_func(&input_ids,
                                             attn_mask_value,
                                             num_beams,
                                             pad_token_id,
                                             sequence_lengths,
  ...
  OrtAllocator *pinned_allocator = allocator_; // session_state_->GetAllocator(provider->GetOrtDeviceByMemType(OrtMemTypeCPU));
  const OrtMemoryInfo& location = *default_allocator.Info(&default_allocator);
  add_to_feeds_func(ort_stream,
                    {expanded_input_ids.get(), expanded_position_ids.get(), expanded_attention_mask.get()},
                    feeds,
                    buffer,
                    &default_allocator,
                    pinned_allocator,
  ...
  auto past_type = IsOutputFloat16() ? Ort::TypeToTensorType<Ort::Float16_t>::type : Ort::TypeToTensorType<float>::type;
  if (!past_present_share_buffer_) {
    // Initialize empty past state
    TensorShape past_shape{2, batch_size * num_beams, num_heads, 0, head_size};
    std::unique_ptr<OrtValue> empty_past=OrtValue::CreateTensor(default_allocator, past_shape.GetDims().data(), past_shape.GetDims().size(), past_type);
//    Tensor::InitOrtValue(past_type, past_shape, default_allocator, empty_past);
    ...
    for (int i = first_past_input_index_; i < num_subgraph_inputs; ++i) {
      feeds.push_back(std::move(empty_past));
    }
  } else {
    // Past state feeds
    ...
    for (int i = first_past_input_index_; i < past_end_iter; ++i) {
      std::unique_ptr<OrtValue> past_tensor = OrtValue::CreateTensor(default_allocator, past_shape.GetDims().data(), past_shape.GetDims().size(), past_type);
      //      Tensor::InitOrtValue(past_type, past_shape, default_allocator, past_tensor);
      feeds.push_back(std::move(past_tensor));
    }

    // Past sequence length feed
    AppendPastSequenceLength(feeds, cpu_allocator, 0);

    // Add beam search specific inputs
    if (need_cache_indir) {
      AppendBeamWidthAndCacheIndir(feeds, cpu_allocator, &default_allocator, batch_size, num_beams,
...
void GptSubgraph::Validate(const std::vector<const NodeArg*>& subgraph_inputs,
