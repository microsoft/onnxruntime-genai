  ...
  void Init(OrtAllocator* cpu_allocator,
    ...
    if (is_cuda) {
      ...
      if (has_decoder_masked_self_attention) {
        ...
        auto temp=OrtValue::CreateTensor<T>(allocator, staging_for_past_state_reorder_buffer_shape);
...
class GreedySearchBase : public GenerateBase {
 public:
  ...
  // Validate inputs.
  void CheckInputs(const OpKernelContextInternal& context) override;

 protected:
  // Process logits and append next tokens to sequences.
  void GenerateNextToken(const OrtValue& logits,
...
#if 0
template <typename T, typename ParametersT>
void GreedySearchBase<T, ParametersT>::CheckInputs(const OpKernelContextInternal& context) {
  // Input shapes:
...
#if 0
template <typename T, typename ParametersT>
void GreedySearchBase<T, ParametersT>::Initialize() {
...
template <typename T, typename ParametersT>
void GreedySearchBase<T, ParametersT>::ProcessLogits(
  ...
  process_logits_func_(logits, &greedy_state, &sampling_state, &(greedy_state.sequences), allocator,
                              this->thread_pool_, &this->logits_processors_, parameters_,
                              use_sampling, counter, this->ort_stream_, this->GetConsoleDumper());
}

template <typename T, typename ParametersT>
void GreedySearchBase<T, ParametersT>::GenerateNextToken(
    const OrtValue& logits,
    gsl::span<int32_t>& next_tokens,
    GreedySearchState<T>& greedy_state,
    ISamplingState<T>& sampling_state,
