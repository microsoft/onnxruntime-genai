...
void TopK(const OrtValue& input, const int axis, const unsigned k, bool largest, bool sorted,
            OrtAllocator* allocator,
            Stream* /*stream*/,
            OrtValue& output_values,
            OrtValue& output_indices) {
  auto type = input.GetTensorTypeAndShapeInfo();
  if (type->GetElementType()==Ort::TypeToTensorType<float>::type) {
    assert(false); // TODO
//    return GetTopK<float>(input, axis, k, largest, sorted, allocator, output_values, output_indices);
  }
