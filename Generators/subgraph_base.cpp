...
Subgraph::Subgraph(
  ...
  num_implicit_inputs = 0; //static_cast<int>(node.ImplicitInputDefs().size());

  auto subgraph_inputs = subgraph_in.GetInputNames();
  auto subgraph_outputs = subgraph_in.GetOutputNames();
  ...
  has_decoder_masked_attention_ = false; // How to detect?
#if 0
...
void Subgraph::Setup(const SessionState& session_state,
                       const SessionState& subgraph_session_state) {
  session_state_ = &session_state;
  subgraph_session_state_ = &subgraph_session_state;

  std::vector<std::string_view> feed_names;
  ...
  FeedsFetchesManager::Create(feed_names, subgraph_output_names,
                              /*subgraph_session_state.GetOrtValueNameIdxMap(), */feeds_fetches_manager_);
//  utils::InitializeFeedFetchCopyInfo(subgraph_session_state, *feeds_fetches_manager_);
  feeds_fetches_manager_->SetDeviceCopyChecks(DeviceCopyCheck::NoCopy, DeviceCopyCheck::NoCopy);
...
void Subgraph::GetParameters(const TensorShapeProto* past_shape,
...
void Subgraph::AppendPastSequenceLength(std::vector<std::unique_ptr<OrtValue>>& feeds,
                                        OrtAllocator* cpu_allocator,
                                        const int32_t init_value) {
  assert(false); // Shouldn't need this
#if 0
  int64_t past_seq_len_dims[] = {1};
  TensorShape past_seq_len_shape(&past_seq_len_dims[0], 1);
  std::unique_ptr<OrtValue> past_seq_len_tensor_value = OrtValue::CreateTensor<int32_t>(*cpu_allocator, past_seq_len_shape.GetDims().data(), past_seq_len_shape.GetDims().size());
  //  Tensor::InitOrtValue(DataTypeImpl::GetType<int32_t>(), past_seq_len_shape, cpu_allocator, past_seq_len_tensor_value);
  *past_seq_len_tensor_value->GetTensorMutableData<int32_t>() = init_value;
  feeds.push_back(std::move(past_seq_len_tensor_value));
#endif
}

void Subgraph::AppendBeamWidthAndCacheIndir(std::vector<std::unique_ptr<OrtValue>>& feeds,
                                            OrtAllocator* cpu_allocator,
  ...
  std::unique_ptr<OrtValue> num_beams_tensor_value = OrtValue::CreateTensor<int32_t>(cpu_allocator, num_beams_shape.GetDims().data(), num_beams_shape.GetDims().size());
  //  Tensor::InitOrtValue(DataTypeImpl::GetType<int32_t>(), num_beams_shape, cpu_allocator, num_beams_tensor_value);
  *num_beams_tensor_value->GetTensorMutableData<int32_t>() = static_cast<int32_t>(num_beams);
  feeds.push_back(std::move(num_beams_tensor_value));
