...
- Always raise an issue if `staticmethod` is used in any method added. The use of `staticmethod` should be avoided. If you find a case where it is used, raise an issue to find a better way to implement the functionality without using `staticmethod`.
