...
def _build_for_apple_sysroot(
    ...
    for current_arch in archs:
        ...
        build_command = [
            *base_build_command,
            ...
            "--build_dir=" + os.path.join(build_dir_current_arch, build_config),
        ]
        ...
        framework_dir = os.path.join(
            ...
            (
                ...
                else os.path.join("static_framework", "onnxruntime-genai.framework")
            ),
        )
        ort_libs.append(os.path.join(framework_dir, "onnxruntime-genai"))
    ...
    framework_dir = os.path.join(intermediates_dir, "frameworks", sysroot, "onnxruntime-genai.framework")
    ...
    if sysroot == "macosx" or sysroot == "macabi":
        ...
        lipo_command += ["-output", os.path.join(framework_dir, "Versions", "A", "onnxruntime-genai")]
        ...
        pathlib.Path(os.path.join(framework_dir, "onnxruntime-genai")).symlink_to("Versions/Current/onnxruntime-genai")
        ...
        lipo_command = ["lipo", "-create"]
        lipo_command += ort_libs
        lipo_command += ["-output", os.path.join(framework_dir, "onnxruntime-genai")]
        subprocess.run(lipo_command, shell=False, check=True)
...
def _build_package(args):
    ...
    for sysroot in build_settings["build_osx_archs"]:
        ...
        if not public_headers_path:
            public_headers_path = os.path.join(os.path.dirname(framework_dir), "onnxruntime-genai.framework", "Headers")
    ...
    xcframework_path = os.path.join(xcframework_dir, "onnxruntime-genai.xcframework")
