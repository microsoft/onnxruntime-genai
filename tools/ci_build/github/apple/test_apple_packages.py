...
def _test_apple_packages(args):
    ...
    has_framework = (c_framework_dir / "onnxruntime-genai.framework").exists()
    has_xcframework = (c_framework_dir / "onnxruntime-genai.xcframework").exists()

    if not has_framework and not has_xcframework:
        raise FileNotFoundError(f"{c_framework_dir} does not have onnxruntime-genai.framework/xcframework")

    if has_framework and has_xcframework:
        raise ValueError("Cannot proceed when both onnxruntime-genai.framework and onnxruntime-genai.xcframework exist")

    framework_name = "onnxruntime-genai.framework" if has_framework else "onnxruntime-genai.xcframework"
    ...
    with contextlib.ExitStack() as context_stack:
        ...
        test_proj_path = pathlib.Path(REPO_DIR, "test/platform/apple/apple_package_test")
