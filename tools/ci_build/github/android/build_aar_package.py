...
# We by default will only build 64-bit 4 ABIs. TBD if we need to support any 32-bit ABIs.
...
def _build_aar(args):
    ...
    temp_env["ANDROID_HOME"] = str(Path(args.android_home).resolve(strict=True))
    temp_env["ANDROID_NDK_HOME"] = str(Path(args.android_ndk_path).resolve(strict=True))
