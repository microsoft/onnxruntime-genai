...
Includes a custom SkipLayerNormalization fusion that works WITHOUT shape
inference, enabling Add+LayerNorm fusion even on dynamic-shape models where
ORT's built-in optimizer fails.

Two-stage optimization:
  1. Graph fusion  — Fuse Conformer attention subgraphs into MultiHeadAttention,
                     SkipLayerNormalization, BiasGelu, etc.
                     Falls back to custom Add+LayerNorm → SkipLayerNorm fusion
                     when ORT shape inference fails (dynamic shapes).
...
Usage:
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype fp16
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int4
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int8
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype fp32
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int4 --quant_method k_quant_mixed
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int4 --quant_method k_quant
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int4 --conv_int8
    python optimize_encoder_1.py --model_dir ./onnx_models --output_dir ./out --dtype int4 --quant_method k_quant --lstm_int8
"""
...
def fuse_skip_layer_norm(model: onnx.ModelProto) -> int:
    """Fuse Add + LayerNormalization → SkipLayerNormalization without shape inference.

    Pattern:
        input_a ──┐
                   Add ── LayerNormalization ── output
        input_b ──┘

    Becomes:
        input_a ──┐
                   SkipLayerNormalization ── output
        input_b ──┘

    The SkipLayerNormalization op (com.microsoft domain) computes:
        hidden = input + skip
        output = gamma * (hidden - mean) / sqrt(var + epsilon) + beta

    This fusion is safe when:
      - LayerNormalization's only input is the Add output
      - The Add output has no other consumers (or we also provide the
        skip output from the fused node)
    ...
    # Build output_name -> consumer count
    consumer_count = {}
    for node in graph.node:
        for inp in node.input:
            consumer_count[inp] = consumer_count.get(inp, 0) + 1
    # Also count graph outputs as consumers
    for out in graph.output:
        consumer_count[out.name] = consumer_count.get(out.name, 0) + 1
    ...
    for node in graph.node:
        ...
        ln_input = node.input[0]  # the tensor being normalized
        if ln_input not in output_to_node:
            continue

        add_node = output_to_node[ln_input]
        if add_node.op_type != "Add":
            continue

        # Check if the Add node's output is only consumed by this LayerNorm
        add_out = add_node.output[0]
        add_consumers = consumer_count.get(add_out, 0)
        ...
        # SkipLayerNormalization inputs: input, skip, gamma, beta, bias(optional)
        # Convention: input = Add.input[0], skip = Add.input[1]
        skip_ln_inputs = [
            add_node.input[0],   # input
            add_node.input[1],   # skip
            node.input[1],       # gamma
        ]
        if len(node.input) > 2 and node.input[2]:
            skip_ln_inputs.append(node.input[2])  # beta

        # SkipLayerNormalization outputs:
        #   0: output (normalized)
        #   1: mean (optional)
        #   2: inv_std_var (optional)
        #   3: input_skip_bias_sum (optional, = input + skip before normalization)
        skip_ln_outputs = [node.output[0]]  # normalized output

        # If Add output has other consumers, provide the sum as the 4th output
        if add_consumers > 1:
            skip_ln_outputs.extend(["", ""])  # placeholders for mean, inv_std_var
            skip_ln_outputs.append(add_out)   # input_skip_bias_sum = original Add output

        skip_ln_node = onnx.helper.make_node(
            "SkipLayerNormalization",
            inputs=skip_ln_inputs,
            outputs=skip_ln_outputs,
            name=f"SkipLayerNorm_fused_{fused}",
        ...
        nodes_to_add.append(skip_ln_node)
        fused += 1

    if fused > 0:
        # Rebuild node list: keep non-removed nodes, append fused nodes
...
def stage1_graph_fusion(input_path: str, output_path: str, disable_skip_layer_norm: bool = False) -> bool:
    ...
    options.enable_skip_layer_norm = not disable_skip_layer_norm
    ...
    try:
        ...
        skip_ln_count = fused_stats.get("SkipLayerNormalization", 0) + \
            fused_stats.get("SkipSimplifiedLayerNormalization", 0)
        if skip_ln_count == 0 and not disable_skip_layer_norm:
            print("  WARNING: No SkipLayerNorm ops were fused by ORT optimizer (likely due to dynamic shapes).")
            print("           Applying custom SkipLayerNormalization fusion (shape-inference-free)...")
            skip_ln_fused = fuse_skip_layer_norm(optimized_model.model)
            print(f"           Custom SkipLayerNorm fusions: {skip_ln_fused}")
            if skip_ln_fused > 0:
                fused_stats["SkipLayerNormalization"] = skip_ln_fused
...
def main():
    ...
    parser.add_argument("--no_skip_layer_norm", action="store_true",
                        help="Disable SkipLayerNormalization fusion (needed for re-exported encoder without baked-in drop_extra_pre_encoded)")
    ...
    if not args.skip_fusion:
        fusion_ok = stage1_graph_fusion(str(encoder_path), str(final_path),
                                        disable_skip_layer_norm=args.no_skip_layer_norm)
        if fusion_ok:
