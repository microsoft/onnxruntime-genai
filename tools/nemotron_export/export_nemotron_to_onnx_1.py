...
with batch=1 fixed and dynamic time axes.
...
Streaming mode (--streaming):
  Exports the encoder with cache-aware streaming I/O (batch=1, time dynamic):
    Inputs:  audio_signal[1,T,128], length[1],
             cache_last_channel[1,24,70,1024], cache_last_time[1,24,1024,8],
             cache_last_channel_len[1]
    Outputs: outputs[1,T',1024], encoded_lengths[1],
             cache_last_channel_next[1,24,70,1024], cache_last_time_next[1,24,1024,8],
             cache_last_channel_len_next[1]

Usage:
    python export_nemotron_to_onnx_1.py --output_dir ./onnx_models
    python export_nemotron_to_onnx_1.py --output_dir ./onnx_models --streaming
    python export_nemotron_to_onnx_1.py --output_dir ./onnx_models --chunk_size 0.56 --streaming
    python export_nemotron_to_onnx_1.py --output_dir ./onnx_models --streaming --chunk_size 0.56 --left_chunks 10
"""
...
def parse_args():
    ...
    parser.add_argument("--opset_version", type=int, default=21,
...
def _make_streaming_encoder_wrapper(encoder):
    ...
    class StreamingEncoderWrapper(nn.Module):
        def __init__(self, enc):
            ...
            # Disable baked-in drop_extra_pre_encoded and valid_out_len so
            # the ONNX graph outputs ALL encoder frames.  The C++ runtime
            # handles the frame skipping dynamically (0 for first chunk,
            # drop_extra_pre_encoded for subsequent chunks).
            if hasattr(enc, 'streaming_cfg'):
                enc.streaming_cfg.drop_extra_pre_encoded = 0
                enc.streaming_cfg.valid_out_len = 0
...
def export_model(args):
    ...
    if args.streaming:
        ...
        dummy_audio = torch.randn(batch_size, chunk_frames, mel_features)
    ...
    with torch.no_grad():
        torch.onnx.export(
            ...
            dynamic_axes={
                "targets": {1: "target_len"},
                "decoder_output": {2: "target_len"},
            },
            dynamo=False,
        )
    ...
    with torch.no_grad():
        torch.onnx.export(
            ...
            dynamic_axes={
                ...
                "decoder_output": {1: "target_len"},
                "joint_output": {1: "time", 2: "target_len"},
            },
            dynamo=False,
        )
