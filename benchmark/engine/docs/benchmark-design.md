...
`mixed_workload` first drives the shorter prompts through one sampled token, then admits the long
RULER prompt and starts measurement so the mixed phase contains active decodes plus the long
prefill. It validates that post-admission decode token events precede the long request's first
token.
