...
`capacity_pressure` submits eight prompts that increase toward 128K to test admission behavior
under memory pressure. The current scenario reports admitted and rejected request counts only;
preemption is explicitly deferred.
