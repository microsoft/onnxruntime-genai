...
With nonzero ``--min-adaptive-k``, the fixed-K sweep is replaced by one adaptive run per mode.
...
Spec-Bench dataset (``question.jsonl``); optional instruction-following, math,
long-context, and code suites add task-quality measurements. ``--limit-per-task``
subsamples Spec-Bench and ``--builtin`` uses a small smoke-test set. Results are
reported per task plus an overall geometric-mean speedup. Results are checkpointed
atomically after each completed configuration; large CSVs rotate into numbered parts.
...
parse_suites = suite_utils.parse_suites
load_gsm8k_prompts = suite_utils.load_gsm8k_prompts
load_humaneval_prompts = suite_utils.load_humaneval_prompts
score_completion = suite_utils.score_completion


def classify_quality_transition(baseline_quality, speculative_quality):
    return suite_utils.classify_quality_transition(
        baseline_quality, speculative_quality, "speculative"
    )
...
def _find_qnn_decoder_model(source_dir: str | None, context: str) -> str:
    """Find the only ONNX entrypoint in a filename-less QNN model directory."""
    if not source_dir or not os.path.isdir(source_dir):
        raise ValueError(
            f"{context} requires an existing QNN model directory, got {source_dir!r}")
    ...
    for root, _, files in os.walk(source_dir, followlinks=False):
        for name in files:
            ...
            candidates.append(_safe_relative_path(
                relative, f"{context} QNN decoder candidate"))

    candidates.sort(key=lambda candidate: (candidate.casefold(), candidate))
    if not candidates:
        raise ValueError(
            f"{context} does not define a decoder filename or pipeline, and its "
            f"QNN model directory contains no .onnx files: {source_dir}")
    if len(candidates) != 1:
        raise ValueError(
            f"{context} does not define a decoder filename or pipeline, and its "
            "QNN model directory contains multiple .onnx files. Set "
            f"model.decoder.filename explicitly. Candidates: {', '.join(candidates)}")
    return candidates[0]
...
def _prefix_decoder_paths(decoder, prefix: str, context: str,
    ...
    if not path_count and _normalize_provider_name(provider) == "qnn":
        filename = _find_qnn_decoder_model(source_dir, context)
        decoder["filename"] = f"{prefix}/{filename}"
        print(f"resolved {context} QNN decoder model: {filename}")
        path_count += 1
    if not path_count:
...
def run_once(og, model, ids, max_new, mode, speculative, K, seed,
             min_adaptive_k=0, cooldown=False):
    import numpy as np
    ...
    if speculative:
        ...
        if cooldown:
            speculative_options["cooldown"] = True
...
def print_detailed_run(item, rep, baseline, row):
    ...
    if row["min_adaptive_k"]:
        print(
            f"    adaptive K: start={row['min_adaptive_k']} "
...
def print_progress_summary(config_rows, completed, total, k):
    ...
    if config_rows[0]["min_adaptive_k"]:
        print(
            f"    adaptive K: start={config_rows[0]['min_adaptive_k']} "
    ...
    for line in suite_utils.format_quality_summary_lines(
        config_rows, "standard", "speculative"
    ):
        print(f"    {line}")
    print(flush=True)
...
CSV_COLUMNS = [
    "mode", "seed", "K", "cooldown", "min_adaptive_k", "effective_k",
    "adaptive_k_increases", "adaptive_k_decreases", "adaptive_k_observations",
    "adaptive_k_probes", "adaptive_k_throughput",
    "task", "subcategory", "question_id", "quality_metric", "quality_score_type",
    "quality_score", "baseline_quality_score", "quality_score_delta",
    "quality_prediction", "quality_reference_answer", "quality_detail",
    "quality_transition", "prompt_id", "rep", "decoder",
    "provider", "device", "prompt_tokens", "output_token_budget",
...
def main():
    ...
    ap.add_argument("--k", default="1,2,4,8",
                    help="sweep var 2: fixed max_draft_tokens; ignored when --min-adaptive-k is nonzero")
    ap.add_argument("--min-adaptive-k", type=int, default=0,
                    help="0 disables adaptive K; 1-16 sets its starting width and floor")
    ...
    ap.add_argument("--ifeval-problems", type=int, default=541,
    ...
    ap.add_argument("--math500-problems", type=int, default=500,
    ...
    ap.add_argument("--longbench-problems-per-task", type=int, default=50,
                    help="problems per selected LongBench task; 0 uses all")
    ap.add_argument("--longbench-max-input-tokens", type=int, default=16384,
    ...
    ap.add_argument("--humanevalplus-problems", type=int, default=164,
                    help="number of HumanEval+ problems; 0 uses all 164")
    ...
    ap.add_argument("--livecodebench-release", default="release_v6",
                    help="pinned LiveCodeBench release to select (default: release_v6)")
    ap.add_argument("--code-execution-timeout", "--humaneval-timeout",
                    dest="code_execution_timeout", type=float, default=3.0,
    ...
    if not 0 <= args.min_adaptive_k <= 16:
        ap.error("--min-adaptive-k must be in [0,16]")
    if args.reps < 1:
    ...
    for option, value in count_options.items():
        if value < 0:
            ap.error(f"{option} cannot be negative")
    if args.longbench_max_input_tokens < 512:
        ap.error("--longbench-max-input-tokens must be at least 512")
    if args.code_execution_timeout <= 0:
        ap.error("--code-execution-timeout must be positive")
    ...
    if args.min_adaptive_k:
        ks = ["adaptive"]
        runtime_ks = {"adaptive": args.min_adaptive_k}
    else:
    ...
    if args.min_adaptive_k:
        print(f"adaptive K enabled: start/floor={args.min_adaptive_k}, "
    ...
    for mode in modes:
        for idx, (it, ids) in enumerate(zip(prompt_items, encoded)):
            ...
            prompt_max_new = suite_utils.generation_limit(it, max_new)
            for rep in range(args.reps):
                r = run_once(
                    og, std_model, ids, prompt_max_new, mode, False, 0, args.seed
                )
                ...
                rows.append(dict(
                    ...
                    K="", cooldown="", min_adaptive_k="", effective_k="",
                    ...
                    question_id=it["question_id"], **base_quality,
                    baseline_quality_score=base_quality["quality_score"],
                    quality_score_delta=(
                        0.0 if base_quality["quality_metric"] else ""
                    ),
                    quality_transition="",
    ...
    if args.warmup:
        ...
        for _ in range(args.warmup):
            run_once(
                ...
                runtime_ks[ks[0]], args.seed,
                min_adaptive_k=args.min_adaptive_k, cooldown=args.cooldown
            )
    ...
    for mode in modes:
        for idx, (it, ids) in enumerate(zip(prompt_items, encoded)):
            ...
            for K in ks:
                ...
                for rep in range(args.reps):
                    r = run_once(
                        og, spec_model, ids, prompt_max_new, mode, True,
                        runtime_k, args.seed, min_adaptive_k=args.min_adaptive_k,
                    ...
                    row = dict(
                        ...
                        min_adaptive_k=args.min_adaptive_k,
                        ...
                        quality_score_delta=(
                            ...
                            - float(base_quality["quality_score"])
                ...
                if args.log_level in ("progress", "detailed"):
                    ...
                    if args.min_adaptive_k:
                        print(
                            f"    adaptive K: start={args.min_adaptive_k} "
                            f"final_p50="
...
def print_summary(rows, modes, ks, *, run_ctx=None, mem_baseline=None, mem_spec=None):
    ...
    if run_ctx:
        ...
        if run_ctx.get("min_adaptive_k"):
            print(f"adaptive K: enabled, start/floor={run_ctx['min_adaptive_k']}, "
                  "native maximum=16")
    ...
    for mode in modes:
        for K in ks:
            ...
            if selected[0]["min_adaptive_k"]:
                print(
                    f"    adaptive K: start={selected[0]['min_adaptive_k']} "
