...
def run_once(
    ...
    if ngram_size:
        ...
        if chained_lookup:
            speculative_options["ngram_chained_lookup"] = True
        if cooldown:
...
def main():
    ...
    parser.add_argument("--k", default="2,4,8",
                        help="comma-separated fixed draft-token counts; ignored when --min-adaptive-k is nonzero")
    parser.add_argument(
        "--min-adaptive-k",
        type=int,
        default=0,
    ...
    if not 0 <= args.min_adaptive_k <= 16:
        parser.error("--min-adaptive-k must be in [0, 16]")
    if args.min_adaptive_k:
        draft_lengths = ["adaptive"]
        runtime_draft_lengths = {"adaptive": args.min_adaptive_k}
    else:
    ...
    if args.min_adaptive_k:
        print(
            f"adaptive K enabled: start/floor={args.min_adaptive_k}, "
    ...
    try:
        ...
        if args.chained_lookup:
            probe_options["ngram_chained_lookup"] = True
        if args.cooldown:
            probe_options["cooldown"] = True
