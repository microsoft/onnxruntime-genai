...
std::unique_ptr<Search> CreateSearch(const GeneratorParams& params, const Model& model) {
  ...
  if (params.device_type == DeviceType::DML) {
    if (params.search.num_beams > 1) {
      return std::make_unique<BeamSearch_Dml>(params, model.GetDmlExecutionContext(), model.GetDmlReadbackHeap());
    }
    return std::make_unique<GreedySearch_Dml>(params, model.GetDmlExecutionContext(), model.GetDmlReadbackHeap());
  }
#endif
