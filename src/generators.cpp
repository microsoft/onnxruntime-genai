...
DeviceInterface* GetDeviceInterface(DeviceType type) {
    // if the allocator remains valid for the lifetime of the process it doesn't need to be tied to the session.
    // that requires knowing the implementation details of the allocator and praying those don't change.
  switch (type) {
    default:
    case DeviceType::CPU:
