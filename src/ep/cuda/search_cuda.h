...
struct GreedySearch_Cuda : Search_Cuda {
  ...
  size_t SampleTopK(int k, float t, std::mt19937& rng) override { return SampleTopKTopP(k, 1.0, t, rng); }
  size_t SampleTopP(float p, float t, std::mt19937& rng) override { return SampleTopKTopP(-1, p, t, rng); }
  size_t SampleTopKTopP(int k, float p, float t, std::mt19937& rng) override;
