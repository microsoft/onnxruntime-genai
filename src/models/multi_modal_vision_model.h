...
struct MultiModalVisionModel : Model {
  ...
  std::unique_ptr<State> CreateState(DeviceMemorySpan<int32_t> sequence_lengths,
...
struct EmbeddingState : State {
  ...
  DeviceMemorySpan<float> Run(int current_length, DeviceMemorySpan<int32_t> next_tokens,
                          DeviceMemorySpan<int32_t> next_indices = {}) override;

 private:
  friend struct MultiModalPipelineState;

  void UpdateInputsAndOutputs(DeviceMemorySpan<int32_t> next_tokens);
...
struct DecoderState : State {
  DecoderState(const MultiModalVisionModel& model, DeviceMemorySpan<int32_t> sequence_lengths,
 ...
 private:
  ...
  void UpdateInputsAndOutputs(int current_length, DeviceMemorySpan<int32_t> beam_indices);
...
struct MultiModalPipelineState : State {
  MultiModalPipelineState(const MultiModalVisionModel& model, DeviceMemorySpan<int32_t> sequence_lengths,
 ...
 private:
  void UpdateInputsOutputs(const DeviceMemorySpan<int32_t>& next_tokens, DeviceMemorySpan<int32_t> next_indices,
                           int current_length);
