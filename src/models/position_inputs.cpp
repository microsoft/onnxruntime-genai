...
// TODO(aciddelgado): WE HAVE REMOVED THE INITIALIZATION WITH SEQUENCE LENGTH HERE
PositionInputs::PositionInputs(const Model& model, State& state, RoamingArray<int32_t>& sequence_lengths_unk)
  ...
  // TODO(aciddelgado): what is this? does it break with 0 length?
...
void PositionInputs::Update(int total_length, int new_length) {
  if (has_posid_input_) {
    // Initialize on first update
    if (is_first_update_) {
      position_ids_shape_[1] = new_length;
      if (type_ == Ort::TypeToTensorType<int32_t>)
        CreateAndInitializePositionIDs<int32_t>(position_ids_shape_);
      else
        CreateAndInitializePositionIDs<int64_t>(position_ids_shape_);
    } else {
      // Batch size > 1 case
  ...
  if (has_mask_input_) {
    ...
    if (is_first_update_) {
      ...
      if (type_ == Ort::TypeToTensorType<int32_t>)
        CreateAndInitializeAttentionMask<int32_t>(attention_mask_shape_);
      else
        CreateAndInitializeAttentionMask<int64_t>(attention_mask_shape_);
    } else {
      // Batch size > 1 case
...
void PositionInputs::UpdatePositionIDs(int total_length, int new_kv_length) {
  ...
  // Don't support DML
  if (model_.device_type_ == DeviceType::DML)
    throw std::runtime_error("PositionInputs::UpdatePositionIDs - DML not supported for continuous decoding.");
  ...
  if (position_ids_shape_[1] != new_kv_length) {
    ...
    if (!sb_position_ids_) {
      position_ids_ = OrtValue::CreateTensor(model_.allocator_cpu_, position_ids_shape_, type_);
    } else {
#if USE_CUDA
      position_ids_ = sb_position_ids_->CreateTensorOnStaticBuffer(position_ids_shape_, type_);
      assert(model_.device_type_ == DeviceType::CUDA);
...
// void PositionInputs::UpdatePositionIDs(int current_length, int new_length) {
...
// void PositionInputs::UpdateAttentionMask(int current_length, int new_length) {
//   if (model_.device_type_ != DeviceType::CPU)
...
template <typename T>
void PositionInputs::CreateAndInitializePositionIDs(std::array<int64_t, 2> shape) {
  ...
  const auto* word_id = state_.input_ids_.Get()->GetTensorData<int>();
  ...
  for (int i = 0; i < shape[0]; i++) {
    ...
    // initial_sequence_lengths_[i] = static_cast<int32_t>(abs_position);
  }
...
template <typename T>
void PositionInputs::CreateAndInitializeAttentionMask(std::array<int64_t, 2> shape) {
  // Set attention mask to be 0 for pad tokens, and 1 for all other tokens.
  // Set position id to be 0 for pad tokens, and accumulated sum of mask in a batch for other tokens
...
// template <typename T>
// void PositionInputs::InitializeTensors(std::array<int64_t, 2> shape/*, cpu_span<int32_t> sequence_lengths_unk*/) {
//   // Set attention mask to be 0 for pad tokens, and 1 for all other tokens.
//   // Set position id to be 0 for pad tokens, and accumulated sum of mask in a batch for other tokens
//   auto* mask_data = attention_mask_->GetTensorMutableData<T>();
//   auto* position_data = position_ids_->GetTensorMutableData<T>();
...
//       // sequence_lengths_unk[i * state_.params_->search.num_beams + k] = static_cast<int32_t>(abs_position);
//       initial_sequence_lengths_[i * state_.params_->search.num_beams + k] = static_cast<int32_t>(abs_position);
//     }
//   }
// }
