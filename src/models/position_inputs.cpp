...
Qwen2VLPositionInputs::Qwen2VLPositionInputs(const Model& model, State& state, DeviceSpan<int32_t> sequence_lengths_unk, const std::unique_ptr<NamedTensors>& inputs)
    : model_{model},
      state_{state},
  ...
  // Extract vision tensors from inputs
  if (inputs) {
    if (auto it = inputs->find("image_grid_thw"); it != inputs->end()) {
      image_grid_thw_ = it->second;
    }
    if (auto it = inputs->find("video_grid_thw"); it != inputs->end()) {
      video_grid_thw_ = it->second;
    }
    if (auto it = inputs->find("second_per_grid_ts"); it != inputs->end()) {
      second_per_grid_ts_ = it->second;
    }
  }
}
...
void Qwen2VLPositionInputs::CreateAndInitialize3DPositionIDs(DeviceSpan<int32_t> next_tokens, std::array<int64_t, 3> shape) {
  ...
  cpu_span<int64_t> image_grid_thw_span;
  if (image_grid_thw_) {
    image_grid_thw_span = image_grid_thw_->GetTensorData<int64_t>();
  }
  cpu_span<int64_t> video_grid_thw_span;
  if (video_grid_thw_) {
    video_grid_thw_span = video_grid_thw_->GetTensorData<int64_t>();
  }
  cpu_span<float> second_per_grid_ts_span;
  if (second_per_grid_ts_) {
    // Qwen 2.5 processor outputs float32 for this
    if (second_per_grid_ts_->GetTensorType() != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT)
      throw std::runtime_error("second_per_grid_ts must be float32.");
    second_per_grid_ts_span = second_per_grid_ts_->GetTensorData<float>();
  }
...
std::unique_ptr<PositionInputs> CreatePositionInputs(State& state, DeviceSpan<int32_t> sequence_lengths, const std::string& attention_mask_name, const std::unique_ptr<NamedTensors>& inputs) {
  ...
  if (ModelType::IsQwen2VL(state.model_.config_->model.type)) {
    return std::make_unique<Qwen2VLPositionInputs>(state.model_, state, sequence_lengths, inputs);
  }

  return CreatePositionInputs(state, sequence_lengths, attention_mask_name);
}
