...
void DefaultPositionInputs::CreateAndInitializePositionIDs(DeviceSpan<int32_t> next_tokens, std::array<int64_t, 2> shape,
                                                           int seq_length) {
  // shape[1] may be larger than seq_length when padded for MIGraphX static shapes.
  ...
  // Defensive: callers always supply seq_length >= 1 (Generator::AppendTokens
  // rejects empty input_ids upstream). If that ever changes, skip the per-token
  // population paths entirely — both branches below assume seq_length >= 1
  // (e.g. forming `position_data + ... + seq_length - 1`).
  if (seq_length <= 0) {
    position_data_next[0] = T{0};
  } else if (shape[0] == 1) {
    // batch_size 1 — simple ascending positions
    for (int i = 0; i < seq_length; ++i) {
      position_data[i] = static_cast<T>(i);
    }
    position_data_next[0] = static_cast<T>(seq_length) - 1;
  } else {
    // batch_size > 1 — iterate backwards so right-pad tokens don't shift positions
