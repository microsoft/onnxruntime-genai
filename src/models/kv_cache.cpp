...
KV_Cache_Combined::KV_Cache_Combined(const Model& model, State& state)
    : model_{model},
      ...
      shape_{2, state_.params_->BatchBeamSize(),
  ...
  for (int i = 0; i < layer_count_; ++i) {
    ...
    snprintf(string, std::size(string),
             model.config_->model.decoder.inputs.past_names.c_str(), i);
    input_name_strings_.emplace_back(string);

    snprintf(string, std::size(string),
             model.config_->model.decoder.outputs.present_names.c_str(), i);
  ...
  for (int i = 0; i < layer_count_; ++i) {
    presents_.push_back(
        OrtValue::CreateTensor(*model.allocator_device_, shape_, type_));
  }
}
...
void KV_Cache_Combined::Update(std::span<const int32_t> beam_indices,
                               int current_length) {
  assert(
      state_.params_->search.num_beams == 1 ||
      !beam_indices.empty());  // We require beam_indices if we're a beam search
...
void KV_Cache_Combined::PickPastState(std::span<const int32_t> beam_indices,
                                      int index) {
  ...
  std::unique_ptr<OrtValue> past =
      OrtValue::CreateTensor<ScoreType>(*model_.allocator_device_, shape_);
  auto past_span = std::span<ScoreType>(past->GetTensorMutableData<ScoreType>(),
                                        element_count);
  auto present_span = std::span<const ScoreType>(
      present.GetTensorData<ScoreType>(), element_count);

#if USE_CUDA
  if (model_.device_type_ == DeviceType::CUDA) {
    for (size_t j = 0; j < beam_indices.size(); j++) {
      int32_t beam_index = beam_indices[j];
      auto present_key = present_span.subspan(beam_index * block_size_per_beam,
                                              block_size_per_beam);
      auto present_value =
          present_span.subspan(past_key_size + beam_index * block_size_per_beam,
                               block_size_per_beam);

      auto past_key =
          past_span.subspan(j * block_size_per_beam, block_size_per_beam);
      auto past_value = past_span.subspan(
          past_key_size + j * block_size_per_beam, block_size_per_beam);
      cudaMemcpyAsync(past_key.data(), present_key.data(),
                      present_key.size_bytes(), cudaMemcpyDeviceToDevice,
                      model_.cuda_stream_);
      cudaMemcpyAsync(past_value.data(), present_value.data(),
                      present_value.size_bytes(), cudaMemcpyDeviceToDevice,
                      model_.cuda_stream_);
    }
  } else
#endif
...
KV_Cache::KV_Cache(const Model& model, State& state)
  ...
  if (g_log.enabled && g_log.warning &&
      past_present_share_buffer_ !=
          state_.params_->search.past_present_share_buffer)
    Log("warning",
        "past_present_share_buffer search option set to true, but has been "
        "disabled due to the current configuration. See "
        "https://aka.ms/generate_config for details");
  ...
  for (int i = 0; i < layer_count_; ++i) {
    char string[64];
    snprintf(string, std::size(string),
             model.config_->model.decoder.inputs.past_key_names.c_str(), i);
    input_name_strings_.emplace_back(string);
    snprintf(string, std::size(string),
             model.config_->model.decoder.inputs.past_value_names.c_str(), i);
    input_name_strings_.emplace_back(string);

    snprintf(string, std::size(string),
             model.config_->model.decoder.outputs.present_key_names.c_str(), i);
    output_name_strings_.emplace_back(string);
    snprintf(string, std::size(string),
             model.config_->model.decoder.outputs.present_value_names.c_str(),
             i);
    output_name_strings_.emplace_back(string);
  }
  ...
  if (state_.GetCapturedGraphInfo()) {
    ...
    for (int i = 0; i < layer_count_ * 2; ++i) {
      sb_kv_caches_.push_back(
          state_.GetCapturedGraphInfo()->sb_kv_caches_[i].get());
    }
  }
...
void KV_Cache::Add() {
  ...
  for (int i = 0; i < layer_count_ * 2; ++i) {
    state_.inputs_.push_back(
        empty_past_.get());  // Set empty past here, AddEncoder() & Update()
  ...
  // For shared_past_present, the past & presents never change, so set the
  // inputs to the present values (outputs are already set above)
  if (past_present_share_buffer_) {
    for (int i = 0; i < layer_count_ * 2; ++i) {
...
void KV_Cache::Update(std::span<const int32_t> beam_indices,
                      int current_length) {
  // If we're sharing past & present buffers there is nothing to do here, so
  // early exit
...
void KV_Cache::PickPastState(std::span<const int32_t> beam_indices, int index) {
  ...
  std::unique_ptr<OrtValue> past_value =
      OrtValue::CreateTensor<ScoreType>(*model_.allocator_device_, shape_);
  auto past_span = std::span<ScoreType>(
      past_value->GetTensorMutableData<ScoreType>(), element_count);
  auto present_span = std::span<const ScoreType>(
      present_value.GetTensorData<ScoreType>(), element_count);

#if USE_CUDA
  if (model_.device_type_ == DeviceType::CUDA) {
    for (size_t j = 0; j < beam_indices.size(); j++) {
      int32_t beam_index = beam_indices[j];
      auto present = present_span.subspan(beam_index * block_size_per_beam,
                                          block_size_per_beam);
      auto past =
          past_span.subspan(j * block_size_per_beam, block_size_per_beam);
      cudaMemcpyAsync(past.data(), present.data(), present.size_bytes(),
                      cudaMemcpyDeviceToDevice, model_.cuda_stream_);
    }
  } else
#endif
  {
...
Cross_Cache::Cross_Cache(const Model& model, State& state)
    : model_{model},
      ...
      layer_count_{model_.config_->model.decoder.num_hidden_layers},
      shape_{state_.params_->BatchBeamSize(),
             model.config_->model.decoder.num_key_value_heads, 1500,
             model.config_->model.decoder.head_size} {
...
PagedCacheOrchestrator::PagedCacheOrchestrator(const Model& model, State& state)
    : model_{model},
      ...
      layer_count_{model.config_->model.decoder.num_hidden_layers} {
  if (model_.device_type_ != DeviceType::CUDA) {
    throw std::runtime_error("Paged cache is only supported on CUDA devices");
  }
  auto cache_config = MakeCacheOptions(model, state);
  shape_ = {cache_config.num_blocks_, cache_config.block_size_ *
...
void PagedCacheOrchestrator::Add() {
  ...
  for (int i = 0; i < layer_count_; ++i) {
    ...
    state_.input_names_.push_back(
        (PagedKeyCacheNamePrefix + std::to_string(i)).c_str());
    state_.inputs_.push_back(key_caches_[i].get());
    state_.input_names_.push_back(
        (PagedValueCacheNamePrefix + std::to_string(i)).c_str());
...
void PagedCacheOrchestrator::Update(
    [[maybe_unused]] std::span<const int32_t> beam_indices,
...
std::unique_ptr<CacheManagerInterface> CreateCacheManager(const Model& model,
                                                          State& state) {
