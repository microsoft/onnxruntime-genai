...
SlidingWindowInputIDs::SlidingWindowInputIDs(State& state) : state_{state} {
  ...
  shape_ = {1, model_.config_->model.decoder.sliding_window_key_value_cache->window_size};
  type_ = model_.session_info_->GetInputDataType(name_);
...
void SlidingWindowInputIDs::Update(DeviceSpan<int32_t>& new_tokens) {
  if (window_index_ == 0) {
    ...
    // All prompt token windows have been processed. Now we process the tokens generated by the model.
    // next_tokens -> [f]
