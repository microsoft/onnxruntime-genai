...
MultiModalLanguageModel::MultiModalLanguageModel(std::unique_ptr<Config> config, OrtEnv& ort_env, bool vision, bool speech)
  ...
  if (vision) {
    ...
    const auto& vision_session_options = config_->model.vision.session_options.has_value() ? config_->model.vision.session_options.value() : config_->model.decoder.session_options;
    CreateSessionOptionsFromConfig(vision_session_options, *vision_session_options_);
    AppendSessionProviders(vision_session_options, *vision_session_options_, true, /*disable_graph_capture=*/true);
  ...
  if (speech) {
    ...
    const auto& speech_session_options = config_->model.speech.session_options.has_value() ? config_->model.speech.session_options.value() : config_->model.decoder.session_options;
    CreateSessionOptionsFromConfig(speech_session_options, *speech_session_options_);
    AppendSessionProviders(speech_session_options, *speech_session_options_, true, /*disable_graph_capture=*/true);
  ...
  const auto& embedding_session_options = config_->model.embedding.session_options.has_value() ? config_->model.embedding.session_options.value() : config_->model.decoder.session_options;
  CreateSessionOptionsFromConfig(embedding_session_options, *embedding_session_options_);
  AppendSessionProviders(embedding_session_options, *embedding_session_options_, true, /*disable_graph_capture=*/true);
