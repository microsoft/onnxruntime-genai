...
MultiModalLanguageModel::MultiModalLanguageModel(std::unique_ptr<Config> config, OrtEnv& ort_env, bool vision, bool speech)
  ...
  if (vision) {
    ...
    CreateSessionOptionsFromConfig(config_->model.vision.session_options.has_value() ? config_->model.vision.session_options.value() : config_->model.decoder.session_options, *vision_session_options_, true, /*disable_graph_capture=*/true, config_->model.vision.asset_dir);
    vision_session_ = CreateSession(ort_env, config_->model.vision.filename, vision_session_options_.get(), config_->model.vision.asset_dir);
  }

  if (speech) {
    speech_session_options_ = OrtSessionOptions::Create();
    CreateSessionOptionsFromConfig(config_->model.speech.session_options.has_value() ? config_->model.speech.session_options.value() : config_->model.decoder.session_options, *speech_session_options_, true, /*disable_graph_capture=*/true, config_->model.speech.asset_dir);
    speech_session_ = CreateSession(ort_env, config_->model.speech.filename, speech_session_options_.get(), config_->model.speech.asset_dir);
  }

  embedding_session_options_ = OrtSessionOptions::Create();
  CreateSessionOptionsFromConfig(config_->model.embedding.session_options.has_value() ? config_->model.embedding.session_options.value() : config_->model.decoder.session_options, *embedding_session_options_, true, /*disable_graph_capture=*/true, config_->model.embedding.asset_dir);

  embedding_session_ = CreateSession(ort_env, config_->model.embedding.filename, embedding_session_options_.get(), config_->model.embedding.asset_dir);
  decoder_session_ = CreateSession(ort_env, config_->model.decoder.filename, session_options_.get(), config_->model.decoder.asset_dir);
...
MultiModalPipelineState::MultiModalPipelineState(const MultiModalLanguageModel& model, DeviceSpan<int32_t> sequence_lengths, const GeneratorParams& params)
  ...
  if (vision_state_ != nullptr && model_.config_->model.vision.adapter_filename.has_value() && num_image_tokens_ > 0) {
    fs::path base = model_.config_->model.vision.asset_dir.string().empty() ? model_.config_->config_path : model_.config_->model.vision.asset_dir;
    const auto lora_adapter = (base / fs::path(*model_.config_->model.vision.adapter_filename)).string();
    ...
    fs::path base = model_.config_->model.speech.asset_dir.string().empty() ? model_.config_->config_path : model_.config_->model.speech.asset_dir;
    const auto lora_adapter = (base / fs::path(*model_.config_->model.speech.adapter_filename)).string();
