...
void ExtraInputs::Update() {
  // Reserve extra inputs when received at runtime (from set_inputs)
  extra_inputs_.reserve(state_.extra_inputs_.size());

  if (state_.GetCapturedGraphInfo()) {
    owned_extra_inputs_.reserve(state_.extra_inputs_.size());

    for (int i = 0; i < state_.extra_inputs_.size(); ++i) {
      auto type_and_shape_info = state_.extra_inputs_[i].tensor->ort_tensor_->GetTensorTypeAndShapeInfo();
      const auto& input_name = state_.extra_inputs_[i].name;
    ...
    for (auto& extra_input : state_.extra_inputs_) {
  ...
  for (int i = 0; i < state_.extra_inputs_.size(); ++i) {
    state_.input_names_.push_back(state_.extra_inputs_[i].name.c_str());
  ...
  for (int i = 0; i < sb_extra_inputs_.size(); ++i) {
    ...
    switch (model_.device_type_) {
      ...
      case DeviceType::DML: {
        ...
        auto source = std::span(state_.extra_inputs_[i].tensor->ort_tensor_->GetTensorData<const uint8_t>(), copy_size_in_bytes);
      ...
      case DeviceType::CUDA: {
        cudaMemcpyAsync(
            ...
            state_.extra_inputs_[i].tensor->ort_tensor_->GetTensorMutableRawData(),
            copy_size_in_bytes,
