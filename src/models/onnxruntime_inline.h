...
inline void OrtThreadingOptions::SetGlobalSpinControl(bool allow_spinning) {
...
inline const T* OrtValue::GetTensorData() const {
  T* out;
  Ort::ThrowOnError(Ort::api->GetTensorMutableData(const_cast<OrtValue*>(this), static_cast<void**>(&out)));
  return out;
}
...
inline void OrtValue::FillStringTensor(const char* const* s, size_t s_len) {
  Ort::ThrowOnError(Ort::api->FillStringTensor(this, s, s_len));
}

inline void OrtValue::FillStringTensorElement(const char* s, size_t index) {
  Ort::ThrowOnError(Ort::api->FillStringTensorElement(this, s, index));
}

inline void* OrtValue::GetTensorMutableRawData() {
  void* out;
...
T* OrtValue::GetTensorMutableData() {
  T* out;
  Ort::ThrowOnError(Ort::api->GetTensorMutableData(this, static_cast<void**>(&out)));
  return out;
}
...
T& OrtValue::At(const std::vector<int64_t>& location) {
  ...
  T* out;
  Ort::ThrowOnError(Ort::api->TensorAt(this, location.data(), location.size(), static_cast<void**>(&out)));
  return *out;
}

#if !defined(DISABLE_SPARSE_TENSORS)
inline void OrtValue::UseCooIndices(int64_t* indices_data, size_t indices_num) {
  Ort::ThrowOnError(Ort::api->UseCooIndices(this, indices_data, indices_num));
}

inline void OrtValue::UseCsrIndices(int64_t* inner_data, size_t inner_num, int64_t* outer_data, size_t outer_num) {
  Ort::ThrowOnError(Ort::api->UseCsrIndices(this, inner_data, inner_num, outer_data, outer_num));
}

inline void OrtValue::UseBlockSparseIndices(const OrtShape& indices_shape, int32_t* indices_data) {
