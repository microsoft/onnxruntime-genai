...
struct ParakeetConfig {
  ...
  int chunk_samples{12800};  // 0.8 seconds at 16kHz (NeMo recommended default)
  int subsampling_factor{8};
  int max_symbols_per_step{10};
  int left_context_samples{143360};   // ~9.0 seconds of left context
  int right_context_samples{25600};   // 1.6 seconds of right context
...
struct ParakeetDecoderState {
  ...
  // Decoder output from last step: [1, lstm_dim, 1]
  std::unique_ptr<OrtValue> decoder_output;
 ...
 private:
  void RunInitialDecoder(const ParakeetConfig& cfg, OrtAllocator& allocator,
                         OrtSession* decoder_session);
  friend struct ParakeetStreamingASR;
};
