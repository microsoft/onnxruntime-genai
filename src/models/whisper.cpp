...
Whisper_Model::Whisper_Model(std::unique_ptr<Config> config, OrtEnv& ort_env)
  ...
  session_decoder_ = OrtSession::Create(ort_env, (concat_file_path(config_->config_path, config_->model.decoder.filename)).c_str(), session_options_.get());
  session_encoder_ = OrtSession::Create(ort_env, (concat_file_path(config_->config_path, config_->model.encoder_decoder_init.filename)).c_str(), session_options_.get());
