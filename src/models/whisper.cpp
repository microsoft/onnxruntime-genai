...
WhisperModel::WhisperModel(std::unique_ptr<Config> config, OrtEnv& ort_env)
  ...
  const auto& encoder_session_options = config_->model.encoder.session_options.has_value() ? config_->model.encoder.session_options.value() : config_->model.decoder.session_options;
