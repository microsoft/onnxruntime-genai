...
void ParakeetConfig::PopulateFromConfig(const Config& config) {
  ...
  left_context_samples = sp.left_context_samples;
  right_context_samples = sp.right_context_samples;
  ...
  // Decoder I/O names (RNNT/TDT prediction network)
...
void ParakeetDecoderState::Initialize(const ParakeetConfig& cfg, OrtAllocator& allocator) {
  ...
  last_token = static_cast<int64_t>(cfg.blank_id);

  // decoder_output will be initialized on first use via RunInitialDecoder
  decoder_output.reset();
}
...
ParakeetSpeechModel::ParakeetSpeechModel(std::unique_ptr<Config> config, OrtEnv& ort_env)
  ...
  // Populate from genai_config.json
