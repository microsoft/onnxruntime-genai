...
void Model::CreateSessionOptions() {
  const auto create_session_options_from_config = [&](const Config::SessionOptions& config_session_options,
                                                      OrtSessionOptions& session_options, bool is_primary_session_options) {
  ...
  create_session_options_from_config(config_->model.decoder.session_options, *session_options_, true);
  ...
  for (auto& pipeline_model : config_->model.decoder.pipeline) {
    if (pipeline_model.session_options.has_value()) {
      ...
      create_session_options_from_config(*pipeline_model.session_options, *emplaced.first->second, false);
    }
  }
}
