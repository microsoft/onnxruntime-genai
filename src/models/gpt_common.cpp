...
void GetModelParams(GptModelParams& model_params, OrtSession& session)
{
  ...
  model_params.layer_count = static_cast<int>(session.GetOutputCount()) - 1;

  auto past_shape = session.GetInputTypeInfo(3) -> GetTensorTypeAndShapeInfo().GetShape();
  model_params.head_count = static_cast<int>(past_shape[2]);
  model_params.hidden_size = static_cast<int>(past_shape[4]);
}
