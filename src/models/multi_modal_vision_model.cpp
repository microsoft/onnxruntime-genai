...
void VisionState::SetExtraInputs(const std::vector<Input>& extra_inputs, int num_image_tokens) {
  num_image_tokens_ = num_image_tokens;
  extra_inputs_.Update(extra_inputs);
  image_features_.Update(true, num_image_tokens);
}
...
void MultiModalPipelineState::SetExtraInputs(const std::vector<Input>& extra_inputs) {
  num_image_tokens_ = GetNumImageTokens(extra_inputs, model_.config_->model.vision.inputs.pixel_values, model_.config_->model.vision.inputs.image_sizes);
  embedding_state_->num_image_tokens_ = num_image_tokens_;
  vision_state_->SetExtraInputs(extra_inputs, num_image_tokens_);
}
