...
std::unique_ptr<State> DecoderOnly_Model::CreateState(RoamingArray<int32_t> sequence_lengths_unk, const GeneratorParams& params) const {
...
// void DecoderOnly_State::AddInputTokens(const RoamingArray<int32_t>& tokens) {
//   input_ids_.AddInputTokens(tokens, reset_input_);
//   reset_input_ = false;
// }

RoamingArray<float> DecoderOnly_State::Run(int total_length, RoamingArray<int32_t> next_tokens, RoamingArray<int32_t> next_indices) {
  // if (!first_run_) {
...
// TODO(aciddelgado): Transition into a new paradigm
...
// TODO(aciddelgado): update should append, not replace for input_ids. ensure correct next and past lengths 
