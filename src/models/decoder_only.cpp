...
std::unique_ptr<State> DecoderOnly_Model::CreateState(DeviceMemorySpan<int32_t> sequence_lengths, const GeneratorParams& params) const {
  return std::make_unique<DecoderOnly_State>(*this, sequence_lengths, params);
}

DecoderOnly_State::DecoderOnly_State(const DecoderOnly_Model& model, DeviceMemorySpan<int32_t> sequence_lengths_unk, const GeneratorParams& params)
    : State{params, model},
      model_{model},
...
DeviceMemorySpan<float> DecoderOnly_State::Run(int current_length, DeviceMemorySpan<int32_t> next_tokens, DeviceMemorySpan<int32_t> next_indices) {
  if (!first_run_) {
...
void DecoderOnly_State::UpdateInputsOutputs(const DeviceMemorySpan<int32_t>& next_tokens_unk, DeviceMemorySpan<int32_t> beam_indices, int current_length) {
