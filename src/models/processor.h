...
struct Processor {
  ...
  static std::shared_ptr<Processor> Create(Config& config, const SessionInfo& session_info, Model& model) {
    return std::make_shared<ProcessorType>(config, session_info, model);
  }
