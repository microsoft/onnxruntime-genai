...
void ParakeetStreamingASR::LoadVocab() {
  ...
  // Try vocab.txt first (one token per line, 0-indexed); fall back to tokens.txt
  // (sherpa-onnx format: "<token> <id>" per line). We read directly from the
  // file rather than using the tokenizer's Decode() because SPM's Decode strips
  // the leading ▁ space marker for individual tokens, which causes word
  // boundaries to be lost during streaming concatenation.
  vocab_.assign(config_.vocab_size, std::string{});
  ...
  if (f.is_open()) {
    ...
    auto tokens_path = model_.config_->config_path / "tokens.txt";
    std::ifstream tf(tokens_path.string());
    if (tf.is_open()) {
      std::string line;
      while (std::getline(tf, line)) {
        while (!line.empty() && (line.back() == '\r' || line.back() == '\n'))
          line.pop_back();
        // Format: "<token> <id>" — split on the LAST space so token may contain spaces.
        auto sp = line.find_last_of(' ');
        if (sp == std::string::npos) continue;
        std::string tok = line.substr(0, sp);
        int id = std::atoi(line.c_str() + sp + 1);
        if (id >= 0 && id < config_.vocab_size) vocab_[id] = std::move(tok);
      }
    } else {
      // Final fallback: use tokenizer Decode (may lose spaces)
...
std::string ParakeetStreamingASR::RunTDTDecoder(OrtValue* encoder_output,
  ...
  while (t < end_frame) {
    ...
    // ── Joiner expects encoder_outputs [1, hidden_dim, 1] and decoder_outputs [1, dec_dim, 1] ──
    ...
    OrtValue* join_inputs[] = {encoder_frame_raw.get(), decoder_state_.decoder_output.get()};
    ...
    // ── Token prediction: argmax over vocab + blank logits (vocab_size + 1) ──
    const int num_tok_logits = vocab_size + 1;  // includes blank at index vocab_size
    int best_token = 0;
    ...
    if (best_token != blank_id) {
      ...
      auto targets_shape = std::array<int64_t, 2>{1, 1};
      auto targets = OrtValue::CreateTensor(allocator, targets_shape, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32);
      *targets->GetTensorMutableData<int32_t>() = static_cast<int32_t>(best_token);
