...
struct DecoderInputs_Element : JSON::Element {
  ...
  void OnValue(std::string_view name, JSON::Value value) override {
    if (name == "input_ids") {
      ...
      v_.input_hidden_states = JSON::Get<std::string_view>(value);
    } else if (name == "attention_mask") {
      v_.attention_mask = JSON::Get<std::string_view>(value);
    } else if (name == "position_ids") {
      v_.position_ids = JSON::Get<std::string_view>(value);
    ...
    } else if (name == "past_seq_len") {  // Alias for past_sequence_length
      v_.past_sequence_length = JSON::Get<std::string_view>(value);
    ...
    } else if (name == "total_seq_len") {  // Alias for total_sequence_length
      v_.total_sequence_length = JSON::Get<std::string_view>(value);
...
struct DecoderOutputs_Element : JSON::Element {
  ...
  void OnValue(std::string_view name, JSON::Value value) override {
    if (name == "logits") {
      ...
      v_.output_hidden_states = JSON::Get<std::string_view>(value);
...
struct PipelineModel_Element : JSON::Element {
  ...
  void OnValue(std::string_view name, JSON::Value value) override {
    if (name == "filename") {
      ...
      // model_id is already set from the outer key name, but allow it in JSON for clarity
      // Optionally could validate: if (v_.model_id.empty()) v_.model_id = JSON::Get<std::string_view>(value);
...
struct VisionPipelineModel_Element : JSON::Element {
  ...
  void OnValue(std::string_view name, JSON::Value value) override {
    if (name == "filename") {
      ...
      // model_id can be present in JSON but we use the array index or outer key if available
      if (v_.model_id.empty()) {
...
struct VisionPipelineArray_Element : JSON::Element {
  explicit VisionPipelineArray_Element(std::vector<Config::Model::Vision::PipelineModel>& v) : v_{v} {}
...
struct VisionPipeline_Element : JSON::Element {
 ...
 private:
  std::vector<Config::Model::Vision::PipelineModel>& v_;
  VisionPipelineArray_Element array_{v_};
};

struct VisionWindowIndexing_Element : JSON::Element {
  explicit VisionWindowIndexing_Element(Config::Model::Vision::WindowIndexing& v) : v_{v} {}
...
struct Vision_Element : JSON::Element {
  ...
  Element& OnObject(std::string_view name) override {
    ...
    if (name == "window_indexing") {
      ...
      window_indexing_ = std::make_unique<VisionWindowIndexing_Element>(*v_.window_indexing);
 ...
 private:
  ...
  std::unique_ptr<SessionOptions_Element> session_options_;
  std::unique_ptr<RunOptions_Element> run_options_;
  std::unique_ptr<VisionWindowIndexing_Element> window_indexing_;
