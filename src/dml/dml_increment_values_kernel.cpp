...
DmlIncrementValuesKernel::DmlIncrementValuesKernel(
    ...
    : device_(d3d12_device),
      ...
      dtype_(dtype),
      values_resource_(values_resource) {
  constants_.element_count = new_kv_length;
  constants_.start_value = total_length - new_kv_length;
  total_element_count_ = new_kv_length;
  ...
  if (dtype == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
    compute_pso_desc.CS = CD3DX12_SHADER_BYTECODE(DmlIncrementValues_Int32::g_CSMainContinuous, sizeof(DmlIncrementValues_Int32::g_CSMainContinuous));
  } else if (dtype == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64) {
    compute_pso_desc.CS = CD3DX12_SHADER_BYTECODE(DmlIncrementValues_Int64::g_CSMainContinuous, sizeof(DmlIncrementValues_Int64::g_CSMainContinuous));
  } else {
    THROW_HR(E_NOTIMPL);
  }
