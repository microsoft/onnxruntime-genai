...
void* DmlAllocator::DmlAlloc(size_t size_in_bytes) {
  ...
  resources_.emplace(allocation, std::move(resource));
...
void DmlAllocator::GetD3D12ResourceFromAllocation(void* allocation, ID3D12Resource** d3d_resource) {
  *d3d_resource = resources_.at(allocation).Get();
  (*d3d_resource)->AddRef();
}
