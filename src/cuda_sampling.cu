...
SamplingData::SamplingData(unsigned long long random_seed, int batch_size, int vocab_size, cudaStream_t stream) {
    ...
    cub::DeviceSegmentedRadixSort::SortPairsDescending(
        nullptr, temp_storage_bytes, (float *)nullptr, (float *)nullptr, (int *)nullptr, (int *)nullptr,
        vocab_size * batch_size, batch_size, (int *)nullptr, (int *)nullptr, 0, sizeof(float) * 8, stream);
...
template <typename T, typename AccumT> struct MaxFloat {
...
template <typename T, typename AccumT> struct SumExpFloat {
    __device__ __forceinline__ SumExpFloat(AccumT v) : max_k(v) {}

    __device__ __forceinline__ AccumT operator()(AccumT sum, T v) const { return sum + exp((AccumT)v - max_k); }
...
template <typename T, int vec_size> struct alignas(sizeof(T) * vec_size) aligned_vector {
    T val[vec_size];
};
...
__device__ __forceinline__ AccumT SoftmaxReduce(AccumT *smem, AccumT val, const Reduction<AccumT> &r,
                                                AccumT defaultVal) {
...
template <typename T, typename AccumT, typename OutT> struct LogSoftmaxForwardEpilogue {
...
template <typename T, typename AccumT, typename OutT> struct SoftmaxForwardEpilogue {
 ...
 */
template <int ILP, typename scalar_t, typename accum_t, typename outscalar_t,
          template <typename, typename, typename> class Epilogue>
__device__ __forceinline__ void WriteFpropResultsVectorized(int size, const int shift, scalar_t *input,
                                                            outscalar_t *output,
...
template <int ILP, typename scalar_t, typename accum_t, typename outscalar_t,
          template <typename, typename, typename> class Epilogue>
__device__ __forceinline__ void WriteFpropResults(int classes, scalar_t *input, outscalar_t *output,
                                                  Epilogue<scalar_t, accum_t, outscalar_t> epilogue) {
    int offset = threadIdx.x;
...
template <int ILP, typename scalar_t, typename accscalar_t, typename outscalar_t,
          template <typename, typename, typename> class Epilogue>
__global__ void SoftmaxBlockForward(outscalar_t *output, scalar_t *input, int classes, int input_stride,
                                    int output_stride, accscalar_t temperature) {
    ...
    accscalar_t max_k =
        SoftmaxReduce<Max, accscalar_t>(sdata, threadMax, Max<accscalar_t>(), -std::numeric_limits<accscalar_t>::max());
    // reduce all values
    accscalar_t threadExp = IlpReduce<SumExpFloat, ILP, scalar_t, accscalar_t>(
        shift, input, classes, SumExpFloat<scalar_t, accscalar_t>(max_k / temperature), static_cast<accscalar_t>(0));
    accscalar_t sumAll =
        SoftmaxReduce<Add, accscalar_t>(sdata, threadExp, Add<accscalar_t>(), static_cast<accscalar_t>(0));
    Epilogue<scalar_t, accscalar_t, outscalar_t> epilogue(max_k, sumAll);
    if (shift == output_shift) {
        WriteFpropResultsVectorized<ILP, scalar_t, accscalar_t, outscalar_t, Epilogue>(classes, shift, input, output,
                                                                                       epilogue);
    } else {
...
void DispatchBlockwiseSoftmaxForward(cudaStream_t *stream, float *output, const float *input, int softmax_elements,
    ...
    if (is_log_softmax) {
        SoftmaxBlockForward<ILP, float, float, float, LogSoftmaxForwardEpilogue>
            <<<grid, block, block.x * sizeof(float), *stream>>>(output, const_cast<float *>(input), softmax_elements,
                                                                input_stride, output_stride, temperature);
    } else {
...
template <typename T>
void LaunchSortPairs(void *d_temp_storage, size_t temp_storage_bytes, const T *d_keys_in, T *d_keys_out,
                     const int *d_values_in, int *d_values_out, int num_items, int num_segments, int *d_offsets,
                     cudaStream_t stream, bool is_descending) {
    if (is_descending) {
...
template <typename T>
void GetTempStorageSize(const T *d_keys_in, const int *d_values_in, int *d_offsets, int num_items, int num_segments,
                        cudaStream_t stream, bool is_descending, size_t &temp_storage_bytes) {
    if (is_descending) {
        cub::DeviceSegmentedRadixSort::SortPairsDescending(nullptr, temp_storage_bytes, d_keys_in, (T *)nullptr,
                                                           d_values_in, (int *)nullptr, num_items, num_segments,
                                                           d_offsets, d_offsets + 1, 0, sizeof(T) * 8, stream);
    } else {
        cub::DeviceSegmentedRadixSort::SortPairs(nullptr, temp_storage_bytes, d_keys_in, (T *)nullptr, d_values_in,
                                                 (int *)nullptr, num_items, num_segments, d_offsets, d_offsets + 1, 0,
                                                 sizeof(T) * 8, stream);
    }
}
...
template <int max_k>
void LaunchGetTopKSubset(cudaStream_t stream, float *scores_in, float *scores_out, int *indices_out, int vocab_size,
                         int batch_size, int k) {
    dim3 grid(batch_size, 1, 1);
    dim3 block(256, 1, 1);
    GetTopKKernel<max_k, 256>
        <<<grid, block, 0, stream>>>(indices_out, scores_in, scores_out, batch_size, vocab_size, k);
}

// Sets up random thresholds for top p or top k sampling
__global__ void RandomThresholdKernelTopPAndK(curandState *curand_states, float *thresholds, float *prefix_sums,
                                              int batch_size, float p, int k) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
...
// Sets up random thresholds for top p or top k sampling
__global__ void RandomThresholdKernelTopP(curandState *curand_states, float *thresholds, float *prefix_sums,
                                          int batch_size, float p) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
...
// Sets up random thresholds for top p or top k sampling
__global__ void RandomThresholdKernelTopK(curandState *curand_states, float *thresholds, float *prefix_sums,
                                          int batch_size, int k) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
...
void LaunchSampleKernel(SamplingData *data, cudaStream_t stream, float *scores, int *indices, int *index_out,
                        int sample_range, int batch_size, float p = 0.0, int k = -1) {
    dim3 grid(batch_size, 1, 1);
    dim3 block(256, 1, 1);
    // Prefix Sums
    ...
    if (p > 0.0 && k > 1) {
        RandomThresholdKernelTopPAndK<<<int(batch_size / 128) + 1, 128, 0, stream>>>(
            data->curand_states.get(), thresholds.data(), prefix_sums.data(), batch_size, p, k);
    } else if (p > 0.0) {
        RandomThresholdKernelTopP<<<int(batch_size / 128) + 1, 128, 0, stream>>>(
            data->curand_states.get(), thresholds.data(), prefix_sums.data(), batch_size, p);
    } else if (k > 1) {
        RandomThresholdKernelTopK<<<int(batch_size / 128) + 1, 128, 0, stream>>>(
            data->curand_states.get(), thresholds.data(), prefix_sums.data(), batch_size, k);
    }
    SampleKernel<256>
        <<<grid, block, 0, stream>>>(prefix_sums.data(), indices, index_out, sample_range, thresholds.data());
}
...
void SoftmaxAndSort(SamplingData *data, cudaStream_t stream, float *scores_in, float *scores_out, int *indices_out,
                    int vocab_size, int batch_size, float temperature) {
    // Softmax scores
    std::span<float> scores{data->scores_softmaxed.get(), static_cast<size_t>(vocab_size * batch_size)};
    DispatchBlockwiseSoftmaxForward<false>(&stream, scores.data(), const_cast<const float *>(scores_in), vocab_size,
                                           vocab_size, vocab_size, batch_size, temperature);
    ...
    LaunchSortPairs<float>(temp_span.data(), data->temp_storage_bytes, scores.data(), scores_out, indices_in.data(),
                           indices_out, vocab_size * batch_size, batch_size, offsets_gpu.data(), stream,
                           /*is_descending*/ true);
}

void LaunchGetTopKSubsetFullSort(SamplingData *data, cudaStream_t stream, float *scores_in, float *scores_out,
    ...
    LaunchSortPairs<float>(temp_span.data(), data->temp_storage_bytes, scores_in, scores_sorted.data(),
                           indices_in.data(), indices_sorted.data(), vocab_size * batch_size, batch_size,
                           offsets_gpu.data(), stream, /*is_descending*/ true);
}

void GetTopKSubset(SamplingData *data, cudaStream_t stream, float *scores_in, float *scores_out, int *indices_out,
    ...
    DispatchBlockwiseSoftmaxForward<false>(&stream, scores_softmaxed.data(), const_cast<const float *>(scores_in),
                                           vocab_size, vocab_size, vocab_size, batch_size, temperature);
// Get top k subset
#define GetTopK(max_k)                                                                                                 \
    LaunchGetTopKSubset<max_k>(stream, scores_softmaxed.data(), scores_out, indices_out, vocab_size, batch_size, k)
    ...
    if (k <= 4) {
        ...
        LaunchGetTopKSubsetFullSort(data, stream, scores_softmaxed.data(), scores_out, indices_out, vocab_size,
                                    batch_size, k);
    }
}

// Kernel launcher for combined (or seperate) top k and top p sampling; where k is the max number of tokens to sample
// and p is the probability threshold
void GetSample(SamplingData *data, cudaStream_t stream, int32_t *next_token_out, float *scores_in, int vocab_size,
    ...
    if (k > 0 && k < vocab_size) {
        GetTopKSubset(data, stream, scores_in, scores_sorted.data(), indices_sorted.data(), vocab_size, batch_size, k,
                      temperature);
    } else {
        SoftmaxAndSort(data, stream, scores_in, scores_sorted.data(), indices_sorted.data(), vocab_size, batch_size,
                       temperature);
    }
    // Sample kernel
    LaunchSampleKernel(data, stream, scores_sorted.data(), indices_sorted.data(), next_token_out, sample_range,
                       batch_size, p, k);
}
