...
def main():
    ...
    if "cpp" in languages:
        ...
        test_schema = "lora_parameters_utils_test.fbs"
        test_schema_path = test_dir / test_schema

        generate_cpp(flatc, schema_path)

        # generate schema used in unit tests and move to the test dir. file is generated by generate_cpp in SCRIPT_DIR
        generate_cpp(flatc, test_schema_path)
        src_test_schema_h = str(SCRIPT_DIR / test_schema) + ".h"
        dest_test_schema_h = str(test_dir / test_schema) + ".h"
        shutil.move(src_test_schema_h, dest_test_schema_h)
