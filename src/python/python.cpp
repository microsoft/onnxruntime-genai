...
PYBIND11_MODULE(onnxruntime_genai, m) {
  ...
  pybind11::class_<OgaStreamingASR>(m, "StreamingASR")
      ...
      .def("transcribe_chunk", [](OgaStreamingASR& asr, pybind11::array_t<float> audio_chunk) -> std::string {
        ...
        return std::string(result.p_); }, pybind11::arg("audio_chunk"), "Feed a chunk of float32 PCM audio (mono, 16kHz) and get newly transcribed text.")
      .def("flush", [](OgaStreamingASR& asr) -> std::string {
        auto result = asr.Flush();
        return std::string(result.p_); }, "Flush remaining buffered audio. Call after the last transcribe_chunk.")
      .def("get_transcript", [](const OgaStreamingASR& asr) -> std::string {
        auto result = asr.GetTranscript();
        return std::string(result.p_); }, "Get the full transcript accumulated so far.")
      .def("reset", [](OgaStreamingASR& asr) { asr.Reset(); }, "Reset streaming state for a new utterance.");
