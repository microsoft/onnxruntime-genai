...
struct PyGenerator {
  ...
  pybind11::array_t<float> LoadAudioBytes(const std::vector<std::string>& audio_datas) {
    return ToPython(*generator_->LoadAudioBytes(audio_datas));
  }
...
PYBIND11_MODULE(onnxruntime_genai, m) {
  ...
  pybind11::class_<OgaAudios>(m, "Audios")
      ...
      .def("open_bytes", &PyGenerator::LoadAudioBytes);
