...
* `mtp_head_fp16=true` (default `false`) — build the head as a dense fp16 `MoE` op. Highest acceptance rate.
...
* `mtp_head_int4_lmhead=true` (default `false`) — keep the head's draft lm_head at int4 even when the rest of the head is int8. The draft logits are corrected by speculative rejection sampling, so this cannot change output accuracy — only draft acceptance.
* `mtp_emit_hidden=true` (default `false`) — additionally expose the MTP head's final hidden state for debugging or validation.

`mtp_head_fp16` and `mtp_head_quant_type` are mutually exclusive. They can also override a floating-point main model; an FP4 head uses int4 for dense MatMuls because the FP4 schemes apply only to `QMoE` experts.
