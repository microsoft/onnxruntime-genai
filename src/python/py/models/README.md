...
This scenario is for when you want to build a model that uses the `PagedAttention` operator so it can be served by ONNX Runtime GenAI's continuous-batching engine. Every paged build packs all sequences into a flattened token axis (`input_ids` becomes 1D `[num_tokens]`). Qwen hybrid builds keep that token-major layout through recurrent layers by using `com.microsoft.VarlenCausalConvWithState` and `com.microsoft.GatedDeltaNet`. Paged builds store KV in `[num_blocks, block_size, num_key_value_heads, head_size]` buffers and replace `attention_mask` and `position_ids` with `block_table`, `cumulative_sequence_lengths`, and `past_sequence_lengths`. Set `prune_lm_head=true` to project only the final packed hidden state per request and output `[batch_size, vocab_size]`; otherwise logits are `[num_tokens, vocab_size]`.
...
Paged hybrid builds describe sparse paged KV and fixed request state in `model.decoder.state_groups`. Every state group's batch dimension is the scheduled request count, never the packed token count. Legacy models whose every layer uses paged KV omit the manifest and preserve the existing implicit contract. Hybrid builds disable CUDA graph capture because fixed state uses transaction-specific staging addresses; plain paged-KV builds are unaffected. A build that exports a checkpoint window also sets `model.decoder.mixed_batch_checkpoints`, which tells the engine the recurrent operators emit that window per request, so a step that mixes a prefill with decodes can still verify the decodes' drafts.
