...
def is_oci_ref(value) -> bool:
    """Whether ``value`` carries the ``oci://`` scheme.
    ...
    repo id, so guessing would silently hijack existing -m/--model_name values.
    """
    if not value:
        return False
    return str(value).lower().startswith(SCHEME)
