...
class MobileLLMModel(LlamaModel):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        super().__init__(config, io_dtype, onnx_dtype, ep, cache_dir, extra_options)
        # MobileLLM uses standard Llama architecture
        # No additional modifications needed

class BaiChuanModel(Model):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        super().__init__(config, io_dtype, onnx_dtype, ep, cache_dir, extra_options)
        # Baichuan uses standard RoPE and RMSNorm - no special config needed
    
    def make_attention(self, layer_id, attention, root_input):
        """
        Override to handle W_pack weight splitting.
        
        Baichuan uses W_pack instead of separate q_proj, k_proj, v_proj.
        Need to split W_pack into three separate projections for ONNX.
        """
        # Check if W_pack exists (Baichuan's packed QKV weights)
        if hasattr(attention, 'W_pack') and not hasattr(attention, 'q_proj'):
            # Split W_pack into q_proj, k_proj, v_proj
            # W_pack shape: [3 * hidden_size, hidden_size]
            w_pack_weight = attention.W_pack.weight
            hidden_size = w_pack_weight.shape[1]
            
            # Create nn.Linear-like objects with weight and bias attributes
            class LinearWeight:
                def __init__(self, weight):
                    self.weight = weight
                    self.bias = None  # Baichuan attention has no bias
            
            # Split the weights: W_pack is [3*H, H], split along dim 0
            attention.q_proj = LinearWeight(w_pack_weight[:hidden_size, :])
            attention.k_proj = LinearWeight(w_pack_weight[hidden_size:2*hidden_size, :])
            attention.v_proj = LinearWeight(w_pack_weight[2*hidden_size:, :])
        
        # Now call parent make_attention with separated weights
        super().make_attention(layer_id, attention, root_input)
    
    def make_lm_head(self, lm_head):
        """
        Override to handle Baichuan's NormHead.
        
        Baichuan uses NormHead which normalizes the weights but has no bias.
        """
        # Add bias attribute if it doesn't exist (NormHead doesn't have bias)
        if not hasattr(lm_head, 'bias'):
            lm_head.bias = None
        
        # Call parent's make_lm_head
        super().make_lm_head(lm_head)


class Idefics3Model(LlamaModel):
    """
    
    """
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        # Idefics3 has a nested config structure with text_config
        # Extract the text decoder configuration from the nested structure
        if hasattr(config, 'text_config'):
            text_config = config.text_config
            # Copy all text_config attributes to the main config object
            for key in text_config.__dict__:
                if not hasattr(config, key):
                    setattr(config, key, getattr(text_config, key))

        # Call parent LlamaModel constructor with the extracted text config
        super().__init__(config, io_dtype, onnx_dtype, ep, cache_dir, extra_options)

    def make_model(self, input_path):
        """Override to load Idefics3ForConditionalGeneration and extract text decoder"""
        # Make inputs and outputs to ONNX model
        self.make_inputs_and_outputs()
        ...
        # Load Idefics3ForConditionalGeneration model
        print("Loading Idefics3ForConditionalGeneration...")
        extra_kwargs = {"num_hidden_layers": self.num_layers} if "num_hidden_layers" in self.extra_options else {}
        full_model = Idefics3ForConditionalGeneration.from_pretrained(
        ...
        # Structure: full_model.model.text_model contains the language model
        # full_model.lm_head is separate
        if hasattr(full_model, 'model') and hasattr(full_model.model, 'text_model'):
            model = full_model.model.text_model
            lm_head_module = full_model.lm_head
            print(" Found text_model at full_model.model.text_model")
        else:
            raise ValueError("Could not find text_model in Idefics3ForConditionalGeneration structure")
        ...
        # Process LM head separately (it's not part of model.modules())
        if not self.exclude_lm_head:
            print("Reading LM head")
            self.make_lm_head(lm_head_module)
...
def create_model(model_name, input_path, output_dir, precision, execution_provider, cache_dir, **extra_options):
    ...
    if config.architectures[0] == "ChatGLMForConditionalGeneration" or config.architectures[0] == "ChatGLMModel":
        ...
        print("WARNING: Setting `--extra_options exclude_embeds=true` by default.")
        extra_options["exclude_embeds"] = True
        onnx_model = Idefics3Model(config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
        onnx_model.model_type = "idefics3"
    elif config_only:
...
def get_args():
    ...
    print("Valid precision + execution provider combinations are: FP32 CPU, FP32 CUDA, FP16 CUDA, FP16 DML, BF16 CUDA, FP16 TRT-RTX, BF16 TRT-RTX, INT4 CPU, INT4 CUDA, INT4 DML, INT4 WebGPU")
    return args
