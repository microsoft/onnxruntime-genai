...
def create_model(
    ...
    **extra_options,
    ...
    # ── Standalone vision encoder models ──────────────────────────────────
    elif config.architectures[0] in {"ViTModel", "ViTForImageClassification", "ViTForMaskedImageModeling"}:
        onnx_model = ViTModel(config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
    elif config.architectures[0] in {"SiglipVisionModel", "SiglipForImageClassification"}:
        onnx_model = SigLIPModel(config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
    elif config.architectures[0] == "SiglipModel":
        # Full SigLIP model (vision + text) — export vision encoder only
        vision_config = config.vision_config
        # Merge top-level fields into vision_config for VisionModel.__init__
        for key in config.__dict__:
            if not hasattr(vision_config, key):
                setattr(vision_config, key, getattr(config, key))
        onnx_model = SigLIPModel(vision_config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
    elif config.architectures[0] in {"Siglip2VisionModel", "Siglip2ForImageClassification"}:
        onnx_model = SigLIP2Model(config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
    elif config.architectures[0] == "Siglip2Model":
        # Full SigLIP2 model — export vision encoder only
        vision_config = config.vision_config
        for key in config.__dict__:
            if not hasattr(vision_config, key):
                setattr(vision_config, key, getattr(config, key))
        onnx_model = SigLIP2Model(vision_config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)
    elif config.architectures[0] in {"Dinov2Model", "Dinov2ForImageClassification", "Dinov2WithRegistersModel"}:
        onnx_model = DINOv2Model(config, io_dtype, onnx_dtype, execution_provider, cache_dir, extra_options)

    # ── Qwen LLM text models ───────────────────────────────────────────────
        ...
        # Export vision encoder (vision.onnx)
        if not extra_options.get("skip_vision", False):
            print("Exporting Qwen2.5-VL vision encoder (vision.onnx)...")
            vision_model = Qwen25VLVisionModel(
                config.vision_config, io_dtype, onnx_dtype, execution_provider, cache_dir,
                {**extra_options, "filename": "vision.onnx"},
            )
            if not config_only:
                vision_model.make_model(input_path)
                vision_model.save_model(output_dir)
            vision_model.make_genai_config(hf_name, extra_kwargs, output_dir)
            vision_model.save_processing(hf_name, extra_kwargs, output_dir)
        # Export text decoder (model.onnx)
        text_config = config.text_config
        for key in text_config:
            if not hasattr(config, key):
                setattr(config, key, getattr(text_config, key))
        ...
        # Export vision encoder (vision.onnx)
        if not extra_options.get("skip_vision", False):
            print("Exporting Qwen3-VL vision encoder (vision.onnx)...")
