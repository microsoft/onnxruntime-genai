...
class Model:
    ...
    def make_int4_algo_config(self, quant_method: str):
        ...
        # so remove any /mlp/ MatMul overrides that don't exist in MoE models.
        if int4_algo_config is not None and self.moe_attrs.get("num_experts", 0) > 0:
            if hasattr(int4_algo_config, "customized_weight_config"):
                keys_to_remove = [k for k in int4_algo_config.customized_weight_config if "/mlp/" in k]
                for k in keys_to_remove:
                    del int4_algo_config.customized_weight_config[k]
    ...
    def make_fused_moe(self, layer_id, mlp, root_input, shared_expert=None, shared_expert_gate=None):
        """Build a fused MoE subgraph (router + routed experts + optional shared expert).

        Supports both 2-weight (gate_up_proj/down_proj with SwiGLU) and 3-weight
        (w1/w2/w3) expert layouts, routers with or without bias, and an optional
        shared expert path with sigmoid gating.

        Args:
            layer_id: Decoder layer index.
            mlp: MoE module containing the router and expert weights.
                 Router: ``mlp.gate`` (no bias) or ``mlp.router`` (with bias).
                 Experts: ``mlp.experts.gate_up_proj``/``mlp.experts.down_proj`` (2-weight)
                          or ``mlp.experts[i].w1``/``w2``/``w3`` (3-weight).
            root_input: The ONNX input tensor name feeding this MoE block.
            shared_expert: Optional shared expert module (e.g. ``mlp.shared_expert``).
            shared_expert_gate: Optional shared expert gate module (e.g. ``mlp.shared_expert_gate``).
        """
        ...
        # --- Router ---
        router_module = mlp.gate if hasattr(mlp, "gate") else mlp.router
        has_router_bias = hasattr(router_module, "bias") and router_module.bias is not None

        router_basename = f"{basename}/router/MatMul"
        router_matmul_name = self.make_matmul(router_module, router_basename, root_input)

        if has_router_bias:
            router_add_name = f"{basename}/router/Add"
            self.make_add_bias(router_module.bias, router_add_name, root_input=f"{router_matmul_name}/output_0")
            router_out = f"{router_add_name}/output_0"
        else:
            router_out = f"{router_matmul_name}/output_0"

        router_reshape_name = f"{basename}/router/Reshape"
        self.make_reshape(
            router_reshape_name,
            [router_out, f"/model/constants/INT64/{[-1, self.moe_attrs['num_experts']]}"],
            dtype=self.io_dtype,
        ...
        # SwiGLU interleaving: repack [gate|up] to [g0,u0,g1,u1,...] when enabled
        if self.moe_attrs.get("swiglu_fusion", 0) == 1:
            half = raw_gate_up.shape[1] // 2
            gate_part = raw_gate_up[:, :half, :]
            up_part = raw_gate_up[:, half:, :]
            raw_gate_up = torch.stack([gate_part, up_part], dim=2).reshape_as(mlp.experts.gate_up_proj)

        if op_type == "MoE":
            self.make_initializer(raw_gate_up, gate_up_proj_weight, to=self.io_dtype)
            ...
            for i in range(self.moe_attrs["num_experts"]):
                qw1, sc1 = self.make_qmoe_weights(raw_gate_up[i])
                gate_up_qw_list.append(qw1)
                gate_up_sc_list.append(sc1)
                qw2, sc2 = self.make_qmoe_weights(down_proj[i])
        ...
        if hasattr(mlp.experts, "gate_up_proj_bias"):
            ...
            moe_inter = raw_gate_up.shape[1]
            self.make_initializer(torch.zeros(num_e, moe_inter), gate_up_proj_bias, to=self.io_dtype)
        ...
        moe_output = f"{moe_name}/output_0"

        # --- Optional shared expert ---
        if shared_expert is not None:
            shared_output = self.make_shared_expert(
                layer_id, shared_expert, shared_expert_gate, root_input,
            )
            ...
            moe_output = f"{combine_name}/output_0"

        self.layernorm_attrs["skip_input"] = moe_output

    def make_shared_expert(self, layer_id, shared_expert, shared_expert_gate, root_input):
        """Build a shared expert SiLU-MLP with optional sigmoid gating.

        Structure: gate_proj -> SiLU -> Mul(gate, up) -> down_proj -> sigmoid_gate -> Mul
        """
        ...
        self.make_sigmoid(silu_sigmoid_name, f"{gate_matmul}/output_0", self.io_dtype,
                          shape=["batch_size", "sequence_length", shared_inter])
        ...
        # Optional sigmoid gating on the shared expert output
