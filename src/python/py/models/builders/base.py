...
class ModelGraphBuilder(ir_tape.Builder):
    ...
    Provides the identical ``op.Cast``, ``op.Add``, ``op.Constant``, … syntax used in
    ``onnxscript.rewriter.ort_fusions`` (where the same class is called
    ``RewriterContext``).  Node names, output-value names, dtypes, and shapes are all
    auto-generated by the underlying ``Builder``; callers never need to specify them.

    Example::

        x_fp32 = self.op.Cast(x, to=int(ir.DataType.FLOAT))
        result = self.op.Mul(x_fp32, weight)
...
class Model:
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        ...
        self.op = ModelGraphBuilder(self.model.graph)
    ...
    def _bind(
        ...
        """Register a node output under ``name``, replacing any pre-created placeholder.

        This is used after a ``self.op.X(...)`` call to give the auto-created output
        value a meaningful name and register it in ``self.values``.  If a placeholder
        value with the same name was already inserted into ``graph.inputs`` or
        ``graph.outputs`` (e.g. by ``make_inputs_and_outputs``), that placeholder is
        replaced in-place so the graph I/O lists stay consistent.
        """
        value.name = name
        if dtype is not None:
            value.dtype = ir.DataType(dtype) if isinstance(dtype, int) else dtype
        if shape is not None:
            value.shape = ir.Shape(shape) if not isinstance(shape, ir.Shape) else shape
    ...
    def _make_skip_simplified_layer_norm(
        self, root_input, skip_input, weight_name, output_0, output_3, io_dtype, shape
    ):
        ...
        added = self.op.Add(root_val, skip_val)
        if output_3 is not None:
            self._bind(added, output_3, io_dtype, ["batch_size", "sequence_length", self.hidden_size])

        # Compose: feed the Add output into the RMS-norm subgraph.
        self._make_simplified_layer_norm(added, weight_name, output_0, io_dtype, shape=shape)

    # This expansion contrib-op can be updated / deprecated in the future.
    def _make_skip_layer_norm(
        self, root_input, skip_input, weight_name, bias_name, output_0, output_3, io_dtype, shape
    ):
        #                          root_input         skip_input
        #                              |                  |
        #                              +------------------+
        ...
        result = self.op.LayerNormalization(
            added, weight_val, bias_val,
            epsilon=self.layernorm_attrs["epsilon"], axis=-1, stash_type=1,
        )
        self._bind(result, output_0, io_dtype, shape)

    # This expansion contrib-op can be updated / deprecated in the future.
    def _make_simplified_layer_norm(self, root_input, weight_name, output_0, io_dtype, shape):
        ...
        # Resolve inputs: root_input may be an ir.Value (from _make_skip_simplified_layer_norm)
        # or a string name (from the direct call site in make_layernorm).
        x_val = root_input if isinstance(root_input, ir.Value) else self.make_value(root_input)
        weight_val = self.make_value(weight_name)
        ...
        pow_out = self.op.Pow(x_fp32, two)
        ...
        eps = self.op.Constant(value=ir.tensor(np.array(self.layernorm_attrs["epsilon"], dtype=np.float32)))
        add_out = self.op.Add(mean_out, eps)

        sqrt_out = self.op.Sqrt(add_out)

        one = self.op.Constant(value=ir.tensor(np.array(1.0, dtype=np.float32)))
        inv_rms = self.op.Div(one, sqrt_out)

        # Scale input: inv_rms * x_fp32, then cast back to io_dtype, then multiply by weight.
        scaled = self.op.Mul(inv_rms, x_fp32)
        scaled_out = self.op.Cast(scaled, to=int(io_dtype))
        result = self.op.Mul(scaled_out, weight_val)
    ...
    def make_block_sparse_moe(self, layer_id, bsm, root_input):
        ...
        shape_out = self.op.Shape(gate_out)
        ...
        router_probs = self.op.Reshape(gate_out, concat_out)
        self._bind(router_probs, f"{gate_reshape_name}/output_0", self.io_dtype, ["num_rows", self.moe_attrs["num_experts"]])
    ...
    def make_past_key_subgraph(self, basename):
        ...
        shape_out = self.op.Shape(past_key_val)
        idx2 = self.op.Constant(value=ir.tensor(np.array(2, dtype=np.int64)))
        gather = self.op.Gather(shape_out, idx2, axis=0)
        self._bind(gather, f"{basename}/Gather/output_0", ir.DataType.INT64, [])
