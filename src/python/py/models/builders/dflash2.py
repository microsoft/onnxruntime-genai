...
class DFlash2Builder:
    ...
    def _conv_coefficients(self, prefix, x, kernel_weight, rows):
        ...
        proj = self.matmul(f"{prefix}/kernel_projection/MatMul", x, kernel_weight, self.hidden_size,
                           2 * self.taps * self.num_groups, rows)
        flat = self.reshape(f"{prefix}/kernel_projection/Reshape", proj, [-1, 2 * self.taps, self.num_groups],
                            self.io_dtype, ["num_block", 2 * self.taps, self.num_groups])
        outs = [f"{prefix}/kernel_projection/Split/output_{i}" for i in range(2 * self.taps)]
        self.make_node("Split", [flat], outs, name=f"{prefix}/kernel_projection/Split", axis=1,
                       num_outputs=2 * self.taps)
        deltas = []
        for i, o in enumerate(outs):
            self.make_value(o, self.io_dtype, ["num_block", 1, self.num_groups])
            deltas.append(
                self.reshape(f"{prefix}/kernel_projection/delta{i}", o, [-1, self.num_groups, 1],
                             self.io_dtype, ["num_block", self.num_groups, 1])
            )
    ...
    def _grouped_conv(self, prefix, x, deltas, base_kernel, shift_mask, rows):
        ...
        for tap in range(self.taps):
            ...
            coef3 = self.binary(f"Add", f"{prefix}/coef{tap}/Add", base_name, deltas[tap],
                                self.io_dtype, ["num_block", self.num_groups, self.group_size])
            coef = self.reshape(f"{prefix}/coef{tap}/Reshape", coef3, [-1, self.hidden_size],
                                self.io_dtype, ["num_block", self.hidden_size])
            if tap == 0:
                shifted = x
            else:
                ...
                shifted = cat
            term = self.binary("Mul", f"{prefix}/term{tap}/Mul", shifted, coef,
                               self.io_dtype, ["num_block", self.hidden_size])
            if tap > 0:
                term = self.binary("Mul", f"{prefix}/term{tap}/Mask", term, shift_mask,
                                   self.io_dtype, ["num_block", self.hidden_size])
            terms.append(term)
        out = terms[0]
        for i, term in enumerate(terms[1:], start=1):
            out = self.binary("Add", f"{prefix}/sum{i}/Add", out, term,
                              self.io_dtype, ["num_block", self.hidden_size])
        return out
    ...
    def make_model(self):
        ...
        aux = self.unary("Cast", "/dflash2/aux/Cast", "aux_hidden_states", self.io_dtype,
                         ["num_ctx", self.aux_hidden_size], to=self.io_dtype)
        ctx = self.matmul("/dflash2/fc/MatMul", aux, w["fc.weight"],
                          self.aux_hidden_size, self.hidden_size, "num_ctx")
        ctx_n = self.rms_norm("/dflash2/hidden_norm", ctx, w["hidden_norm.weight"], "num_ctx")
        ctx_kv = []
        for i in range(self.num_layers):
            k = self.matmul(f"/dflash2/layers.{i}/ctx_k/MatMul", ctx_n, w[f"layers.{i}.self_attn.k_proj.weight"],
                            self.hidden_size, self.num_kv_heads * self.head_size, "num_ctx",
                            weight_name=f"dflash2.layers.{i}.self_attn.k_proj.weight")
            v = self.matmul(f"/dflash2/layers.{i}/ctx_v/MatMul", ctx_n, w[f"layers.{i}.self_attn.v_proj.weight"],
                            self.hidden_size, self.num_kv_heads * self.head_size, "num_ctx",
        ...
        emb_ext = self.binary("Gather", "/dflash2/embed_tokens/Gather", "model.embed_tokens.weight", "input_ids",
                              self.external_dtype, [rows_q, self.hidden_size])
        emb = self.unary("Cast", "/dflash2/embed_tokens/Cast", emb_ext, self.io_dtype,
        ...
        if self.input_embedding_scale != 1.0:
            ...
            scale_cast = self.unary("Cast", "/dflash2/embed_tokens/ScaleCast", scale, self.io_dtype, [1],
                                    to=self.io_dtype)
            emb = self.binary("Mul", "/dflash2/embed_tokens/Mul", emb, scale_cast,
                              self.io_dtype, [rows_q, self.hidden_size])
        ...
        for i in range(self.num_layers):
            ...
            if residual is None:
                ...
                x, residual = self.skip_rms_norm(f"{p}/input_layernorm", residual, hidden,
                                                 w[f"layers.{i}.input_layernorm.weight"], rows_q)

            # attention_conv.prepare / .finish
            attn_deltas = self._conv_coefficients(f"{p}/attention_conv", x,
                                                  w[f"layers.{i}.attention_conv.kernel_projection.weight"], rows_q)
            x = self._grouped_conv(f"{p}/attention_conv/prepare", x, attn_deltas[0],
                                   w[f"layers.{i}.attention_conv.base_kernel"][0], shift_mask, rows_q)

            attn_out = self._make_attention(i, x, ctx_kv[i], rows_q)
            attn_out = self._grouped_conv(f"{p}/attention_conv/finish", attn_out, attn_deltas[1],
                                          w[f"layers.{i}.attention_conv.base_kernel"][1], shift_mask, rows_q)

            y, residual = self.skip_rms_norm(f"{p}/post_attention_layernorm", residual, attn_out,
                                             w[f"layers.{i}.post_attention_layernorm.weight"], rows_q)

            mlp_deltas = self._conv_coefficients(f"{p}/mlp_conv", y,
                                                 w[f"layers.{i}.mlp_conv.kernel_projection.weight"], rows_q)
            y = self._grouped_conv(f"{p}/mlp_conv/prepare", y, mlp_deltas[0],
                                   w[f"layers.{i}.mlp_conv.base_kernel"][0], shift_mask, rows_q)
            m = self._make_mlp(i, y, w, rows_q)
            hidden = self._grouped_conv(f"{p}/mlp_conv/finish", m, mlp_deltas[1],
                                        w[f"layers.{i}.mlp_conv.base_kernel"][1], shift_mask, rows_q)
    ...
    def _make_attention(self, i, x, ctx_kv, rows_q):
        p = f"/dflash2/layers.{i}"
        w = self._weights
        q = self.matmul(f"{p}/attn/q_proj/MatMul", x, w[f"layers.{i}.self_attn.q_proj.weight"],
                        self.hidden_size, self.num_heads * self.head_size, rows_q)
        k = self.matmul(f"{p}/attn/k_proj/MatMul", x, w[f"layers.{i}.self_attn.k_proj.weight"],
                        self.hidden_size, self.num_kv_heads * self.head_size, rows_q,
                        weight_name=f"dflash2.layers.{i}.self_attn.k_proj.weight")
        v = self.matmul(f"{p}/attn/v_proj/MatMul", x, w[f"layers.{i}.self_attn.v_proj.weight"],
                        self.hidden_size, self.num_kv_heads * self.head_size, rows_q,
                        weight_name=f"dflash2.layers.{i}.self_attn.v_proj.weight")
        ...
        q_all = self.binary("Gather", f"{p}/attn/q_gather", q, "q_row_map", self.io_dtype,
                            ["num_tokens", self.num_heads * self.head_size])
        k_all = self.binary("Gather", f"{p}/attn/k_gather", k_cat, "qkv_row_map", self.io_dtype,
                            ["num_tokens", kv_dim])
        v_all = self.binary("Gather", f"{p}/attn/v_gather", v_cat, "qkv_row_map", self.io_dtype,
                            ["num_tokens", kv_dim])

        q_norm = self.make_initializer(w[f"layers.{i}.self_attn.q_norm.weight"],
                                       f"dflash2.layers.{i}.self_attn.q_norm.weight", to=self.io_dtype)
        k_norm = self.make_initializer(w[f"layers.{i}.self_attn.k_norm.weight"],
                                       f"dflash2.layers.{i}.self_attn.k_norm.weight", to=self.io_dtype)
        ...
        self.make_node(
            ...
            [
                ...
                "cumulative_sequence_lengths", "past_sequence_lengths", "block_table",
                "dflash2.cos_cache", "dflash2.sin_cache",
        ...
        for suffix in ("key", "value"):
            self.make_value(f"present.{i}.{suffix}", self.io_dtype,
                            ["num_blocks", "block_size", self.num_kv_heads, self.head_size])

        block_out = self.binary("Gather", f"{p}/attn/out_gather", attn_out, "block_row_index",
                                self.io_dtype, [rows_q, self.num_heads * self.head_size])
        return self.matmul(f"{p}/attn/o_proj/MatMul", block_out, w[f"layers.{i}.self_attn.o_proj.weight"],
                           self.num_heads * self.head_size, self.hidden_size, rows_q)

    def _make_mlp(self, i, x, w, rows_q):
        p = f"/dflash2/layers.{i}/mlp"
        gate = self.matmul(f"{p}/gate_proj/MatMul", x, w[f"layers.{i}.mlp.gate_proj.weight"],
                           self.hidden_size, self.intermediate_size, rows_q)
        up = self.matmul(f"{p}/up_proj/MatMul", x, w[f"layers.{i}.mlp.up_proj.weight"],
                         self.hidden_size, self.intermediate_size, rows_q)
        ...
        return self.matmul(f"{p}/down_proj/MatMul", prod, w[f"layers.{i}.mlp.down_proj.weight"],
                           self.intermediate_size, self.hidden_size, rows_q)
    ...
    def _make_candidates_and_selector(self, final, w):
        ...
        h3 = self.reshape("/dflash2/select/Reshape", final, [-1, self.block_size, self.hidden_size],
                          self.io_dtype, ["batch_size", self.block_size, self.hidden_size])
        sl = self._out("/dflash2/select/Slice")
        self.make_node("Slice", [h3, self.const([1]), self.const([self.block_size]), self.const([1])], [sl],
                       name="/dflash2/select/Slice")
        self.make_value(sl, self.io_dtype, ["batch_size", n_spec, self.hidden_size])
        hsel = self.reshape("/dflash2/select/Flatten", sl, [-1, self.hidden_size],
                            self.io_dtype, ["num_sample", self.hidden_size])

        # LM head (the target's, shared on disk) -> top-k candidates per position.
        logits = self._make_lm_head(hsel)
        logits32 = self.unary("Cast", "/dflash2/topk/Cast", logits, ir.DataType.FLOAT,
                              ["num_sample", self.vocab_size], to=ir.DataType.FLOAT)
        vals, idx = "/dflash2/topk/values", "/dflash2/topk/indices"
        self.make_node("TopK", [logits32, self.const([top_k])], [vals, idx], name="/dflash2/topk/TopK",
        ...
        cand = self.reshape("/dflash2/topk/cand", idx, [-1, n_spec, top_k],
                            ir.DataType.INT64, ["batch_size", n_spec, top_k])
        unary_logits = self.reshape("/dflash2/topk/unary", vals, [-1, n_spec, top_k],
                                    ir.DataType.FLOAT, ["batch_size", n_spec, top_k])

        # Low-rank edge scores between position l-1 and l.
        hp = self.matmul("/dflash2/selector/hidden_projection/MatMul", hsel,
                         w["candidate_selector.hidden_projection.weight"], self.hidden_size, rank, "num_sample")
        hp3 = self.reshape("/dflash2/selector/hp", hp, [-1, n_spec, 1, rank],
                           self.io_dtype, ["batch_size", n_spec, 1, rank])

        ids2 = self.reshape("/dflash2/selector/ids", "input_ids", [-1, self.block_size],
                            ir.DataType.INT64, ["batch_size", self.block_size])
        anchor = self._out("/dflash2/selector/anchor")
        self.make_node("Slice", [ids2, self.const([0]), self.const([1]), self.const([1])], [anchor],
                       name="/dflash2/selector/anchor")
        self.make_value(anchor, ir.DataType.INT64, ["batch_size", 1])
        anchor3 = self.reshape("/dflash2/selector/anchor3", anchor, [-1, 1, 1],
                               ir.DataType.INT64, ["batch_size", 1, 1])
        anchor_tiled = self.binary("Tile", "/dflash2/selector/anchor_tile", anchor3, self.const([1, 1, top_k]),
                                   ir.DataType.INT64, ["batch_size", 1, top_k])
        prev_tail = self._out("/dflash2/selector/prev_tail")
        self.make_node("Slice", [cand, self.const([0]), self.const([n_spec - 1]), self.const([1])], [prev_tail],
        ...
        self.make_initializer(w["candidate_selector.predecessor_codebook"],
                              "dflash2.candidate_selector.predecessor_codebook", to=self.io_dtype)
        self.make_initializer(w["candidate_selector.successor_codebook"],
                              "dflash2.candidate_selector.successor_codebook", to=self.io_dtype)
        pred = self.binary("Gather", "/dflash2/selector/pred_gather",
                           "dflash2.candidate_selector.predecessor_codebook", prev,
                           self.io_dtype, ["batch_size", n_spec, top_k, rank])
        succ = self.binary("Gather", "/dflash2/selector/succ_gather",
                           "dflash2.candidate_selector.successor_codebook", cand,
                           self.io_dtype, ["batch_size", n_spec, top_k, rank])
        pw = self.binary("Mul", "/dflash2/selector/pw", pred, hp3,
                         self.io_dtype, ["batch_size", n_spec, top_k, rank])
        succ_t = self.unary("Transpose", "/dflash2/selector/succ_t", succ, self.io_dtype,
                            ["batch_size", n_spec, rank, top_k], perm=[0, 1, 3, 2])
        pair = self.binary("MatMul", "/dflash2/selector/pair", pw, succ_t,
                           self.io_dtype, ["batch_size", n_spec, top_k, top_k])
        pair32 = self.unary("Cast", "/dflash2/selector/pair_cast", pair, ir.DataType.FLOAT,
                            ["batch_size", n_spec, top_k, top_k], to=ir.DataType.FLOAT)
        unary4 = self.reshape("/dflash2/selector/unary4", unary_logits, [-1, n_spec, 1, top_k],
                              ir.DataType.FLOAT, ["batch_size", n_spec, 1, top_k])
        ...
        self.make_node("Cast", [cand], ["draft_candidate_ids"], name="/dflash2/selector/cand_cast",
    ...
    def _make_lm_head(self, root):
        ...
        if weight.dtype != torch.float8_e4m3fn:
            name = "/lm_head/MatMul"
            return self.matmul(name, root, weight, self.hidden_size, self.vocab_size, "num_sample",
        ...
        root = self.unary("Cast", "/lm_head/Cast", root, self.external_dtype,
                          ["num_sample", self.hidden_size], to=self.external_dtype)
        self.make_initializer(weight.contiguous(), "lm_head.MatMul.fp8_weight")
        self.make_initializer(scale.reshape(self.vocab_size, 1), "lm_head.MatMul.fp8_weight_scale",
        ...
        for i in range(self.num_layers):
            for suffix in ("key", "value"):
                self.graph.inputs.append(
                    self.make_value(f"past_key_values.{i}.{suffix}", self.io_dtype,
                                    ["num_blocks", "block_size", self.num_kv_heads, self.head_size])
                )
    ...
    def save_model(self, out_dir):
        ...
        with tqdm() as pbar:
            ...
            ir.save(self.model, out_path, external_data=os.path.basename(data_path),
