...
class DSparkBuilder:
    ...
    def __init__(
        ...
        self.block_size = int(num_draft_tokens) if num_draft_tokens else int(cfg["block_size"])
    ...
    def make_model(self):
        ...
        aux = self.unary("Cast", "/dspark/aux/Cast", "aux_hidden_states", self.io_dtype,
                         ["num_ctx", self.aux_hidden_size], to=self.io_dtype)
        ctx = self.matmul("/dspark/fc/MatMul", aux, w["fc.weight"],
        ...
        for i in range(self.num_layers):
            k = self.matmul(f"/dspark/layers.{i}/ctx_k/MatMul", ctx_n, w[f"layers.{i}.self_attn.k_proj.weight"],
                            self.hidden_size, self.num_kv_heads * self.head_size, "num_ctx",
                            weight_name=f"dspark.layers.{i}.self_attn.k_proj.weight")
            v = self.matmul(f"/dspark/layers.{i}/ctx_v/MatMul", ctx_n, w[f"layers.{i}.self_attn.v_proj.weight"],
                            self.hidden_size, self.num_kv_heads * self.head_size, "num_ctx",
        ...
        emb_ext = self.binary("Gather", "/dspark/embed_tokens/Gather", "model.embed_tokens.weight", "input_ids",
                              self.external_dtype, [rows_q, self.hidden_size])
        hidden = self.unary("Cast", "/dspark/embed_tokens/Cast", emb_ext, self.io_dtype,
    ...
    def _make_attention(self, i, x, ctx_kv, rows_q):
        ...
        q_norm = self.make_initializer(w[f"layers.{i}.self_attn.q_norm.weight"],
                                       f"dspark.layers.{i}.self_attn.q_norm.weight", to=self.io_dtype)
        k_norm = self.make_initializer(w[f"layers.{i}.self_attn.k_norm.weight"],
                                       f"dspark.layers.{i}.self_attn.k_norm.weight", to=self.io_dtype)
        ...
        self.make_node(
            ...
            [
                ...
                "dspark.cos_cache", "dspark.sin_cache",
    ...
    def _make_candidates_and_markov(self, final, w):
        ...
        logits32 = self.unary("Cast", "/dspark/topk/Cast", logits, ir.DataType.FLOAT,
                              ["num_block", self.vocab_size], to=ir.DataType.FLOAT)
        vals, idx = "/dspark/topk/values", "/dspark/topk/indices"
        self.make_node("TopK", [logits32, self.const([top_k])], [vals, idx], name="/dspark/topk/TopK",
        ...
        cand = self.reshape("/dspark/topk/cand", idx, [-1, n_spec, top_k],
                            ir.DataType.INT64, ["batch_size", n_spec, top_k])
        unary_logits = self.reshape("/dspark/topk/unary", vals, [-1, n_spec, top_k],
        ...
        ids2 = self.reshape("/dspark/markov/ids", "input_ids", [-1, self.block_size],
        ...
        self.make_node("Slice", [ids2, self.const([0]), self.const([1]), self.const([1])], [anchor],
                       name="/dspark/markov/anchor")
        self.make_value(anchor, ir.DataType.INT64, ["batch_size", 1])
        anchor3 = self.reshape("/dspark/markov/anchor3", anchor, [-1, 1, 1],
                               ir.DataType.INT64, ["batch_size", 1, 1])
        anchor_tiled = self.binary("Tile", "/dspark/markov/anchor_tile", anchor3, self.const([1, 1, top_k]),
        ...
        pred = self.binary("Gather", "/dspark/markov/pred_gather", "dspark.markov_w1", prev,
                           self.io_dtype, ["batch_size", n_spec, top_k, rank])
        succ = self.binary("Gather", "/dspark/markov/succ_gather", "dspark.markov_w2", cand,
                           self.io_dtype, ["batch_size", n_spec, top_k, rank])
        ...
        pred32 = self.unary("Cast", "/dspark/markov/pred_cast", pred, ir.DataType.FLOAT,
                            ["batch_size", n_spec, top_k, rank], to=ir.DataType.FLOAT)
        succ32 = self.unary("Cast", "/dspark/markov/succ_cast", succ, ir.DataType.FLOAT,
                            ["batch_size", n_spec, top_k, rank], to=ir.DataType.FLOAT)
        succ_t = self.unary("Transpose", "/dspark/markov/succ_t", succ32, ir.DataType.FLOAT,
                            ["batch_size", n_spec, rank, top_k], perm=[0, 1, 3, 2])
        pair32 = self.binary("MatMul", "/dspark/markov/pair", pred32, succ_t,
                             ir.DataType.FLOAT, ["batch_size", n_spec, top_k, top_k])
        unary4 = self.reshape("/dspark/markov/unary4", unary_logits, [-1, n_spec, 1, top_k],
        ...
        self.make_node("Cast", [cand], ["draft_candidate_ids"], name="/dspark/markov/cand_cast",
    ...
    def _make_lm_head(self, root):
        ...
        if weight.dtype != torch.float8_e4m3fn:
            name = "/lm_head/MatMul"
            return self.matmul(name, root, weight, self.hidden_size, self.vocab_size, "num_block",
        ...
        root = self.unary("Cast", "/lm_head/Cast", root, self.external_dtype,
                          ["num_block", self.hidden_size], to=self.external_dtype)
