...
class WhisperEncoder(Model):
    ...
    def make_inputs_and_outputs(self):
        ...
        self.input_shapes = {
            "audio_features": ["batch_size", self.num_mel_bins, 3000]
        ...
        self.output_shapes = {
            "hidden_states": [
                ...
                self.hidden_size,
            ],  # ['batch_size', 'num_frames / 2', 'hidden_size']
            "present_key_cross": [
                "batch_size",
                self.num_attn_heads,
                self.max_source_positions,
    ...
    def make_preprocessing_nodes(self):
        ...
        self.make_value(
            gelu_2_output, dtype=self.io_dtype, shape=["batch_size", self.hidden_size, self.max_source_positions]
        )
...
class WhisperDecoder(Model):
    ...
    def make_preprocessing_nodes(self):
        ...
        self.make_gather(
            gather_0_name,
            [f"{shape_0_name}/output_0", "/model/constants/INT64/1"],
        ...
        self.make_add(
            add_1_name,
            [f"{gather_0_name}/output_0", f"{past_key_gather_name}/output_0"],
            dtype=ir.DataType.INT64,
            shape=[],
        )
        ...
        self.make_unsqueeze(
            unsqueeze_0_name,
            [f"{add_1_name}/output_0", "/model/constants/INT64/[0]"],
            dtype=ir.DataType.INT64,
            shape=[1],
        )
        ...
        self.make_unsqueeze(
            unsqueeze_1_name,
            [f"{past_key_gather_name}/output_0", "/model/constants/INT64/[0]"],
            dtype=ir.DataType.INT64,
            shape=[1],
        )
...
class WhisperModel(Model):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        ...
        self.encoder = WhisperEncoder(
            copy.deepcopy(config), io_dtype, onnx_dtype, ep, cache_dir, copy.deepcopy(extra_options)
        )
                    ...
                    },
                    {
                        "operation": {
                            ...
                            "attrs": {
                                ...
                                "hop_length": 160,
                                "n_fft": 400,
                                "n_mel": self.encoder.num_mel_bins,
                            },
                        }
                    },
                ]
            ...
            "model": {
                ...
                "decoder": {
                    "session_options": {
                        ...
                        "provider_options": (
                            [{self.decoder.ep: self.decoder.ep_attrs[self.decoder.ep]}]
                            if self.decoder.ep != "cpu"
                            else []
                        ),
                    },
                ...
                "encoder": {
                    "session_options": {
                        "log_id": "onnxruntime-genai",
                        "provider_options": (
                            [{self.encoder.ep: self.encoder.ep_attrs[self.encoder.ep]}]
                            if self.encoder.ep != "cpu"
                            else []
                        ),
                    },
