...
EngineDependencies Engine::CreateDependencies(std::shared_ptr<Model> model) {
  ...
  if (!model->config_->model.dflash2.filename.empty()) {
    ...
    // Loaded before the main pool so the pool's free-memory measurement already excludes the
    // drafter's weights.
    ...
    if (pool_blocks != 0) {
      // A windowed drafter's footprint depends on the batch size, not the context length, so it
      // can be allocated up front and simply be part of what the pool sees as already used.
      dflash2_drafter =
          std::make_unique<Dflash2Drafter>(dflash2_model, paged_block_size, pool_blocks);
    } else {
      // A full-attention drafter needs one block per block the main cache holds, so it is billed
      // per block and sized from the pool's own block count below.
  ...
  if (dflash2_model && !dflash2_drafter) {
    // The drafter's block granularity matches the main cache's, but a request's query block can
    // spill one block past what the target has committed, so keep a per-request slack block.
    dflash2_drafter = std::make_unique<Dflash2Drafter>(
        dflash2_model, static_cast<size_t>(model->config_->engine.dynamic_batching->block_size),
...
SpeculativeStats Engine::GetSpeculativeStats() const noexcept {
  ...
  stats.fixed_state_nonmonotonic_slot_fallbacks = fixed_state.nonmonotonic_slot_fallbacks;
  stats.fixed_state_gapped_slot_fallbacks = fixed_state.gapped_slot_fallbacks;
  stats.fixed_state_mixed_active_bank_fallbacks = fixed_state.mixed_active_bank_fallbacks;
  stats.fixed_state_capsule_bytes_allocated = fixed_state.capsule_bytes_allocated;
  stats.fixed_state_capsule_valid_prefix_bytes = fixed_state.capsule_valid_prefix_bytes;
  stats.fixed_state_capsule_suffix_bytes_not_written = fixed_state.capsule_suffix_bytes_not_written;
