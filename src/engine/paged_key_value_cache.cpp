...
void PagedKeyValueCache::UpdateState(State& state, const std::vector<std::shared_ptr<Request>>& requests) {
  ...
  if (state.inputs_.empty()) {
    ...
    state.inputs_.reserve(cache.size() * 2 + 1);
    state.input_names_.reserve(cache.size() * 2 + 1);
  }
  ...
  state.inputs_.push_back(block_tables.first);
  state.input_names_.push_back(block_tables.second);
}
