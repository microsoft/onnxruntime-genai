...
OgaResult* OgaSelenaTest(OgaTensor** result, const int32_t *sequences_data, size_t sequences_size, const char* model_path, const char* unet_model_path, const char* vae_model_path) {
  OGA_TRY
  ...
  auto final_output_tensor = std::make_shared<Generators::Tensor>(std::move(vae_output_tensor[0]));
