...
/**
 * \brief For streaming RNNT models: process the current mel input through encoder+decoder
 * and return newly decoded text. Call OgaGenerator_SetModelInput with "audio_features" first.
 * \param[in] generator The generator to run.
 * \param[out] text Pointer to store the decoded text. Caller must free with OgaDestroyString.
 * \return OgaResult containing the error message if the generation failed.
 */
OGA_EXPORT OgaResult* OGA_API_CALL OgaGenerator_GenerateNextTokens(OgaGenerator* generator, const char** text);
